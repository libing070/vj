$(document).ready(function () {
    ...
    // 日志记录
    get_userBehavior_log('专题', '员工异常业务操作', '', '访问');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            function fenxiRightControl() {
                ...
                // 统计分析-统计报表权限控制
                if (data.ygycpmhz || data.ygycqtcd) {
                    // 添加统计报表tab按钮-并处于活动状态
                    ...
                    if (data.ygycpmhz && data.ygycqtcd) {
                        ...
                        $('#fenxiFourNav1Con .five_nav>ul').append('<li class="active" data-target="#fenxiFourNav1FiveNav1Con" data-toggle="tab">排名汇总</li>' +
                            '<li data-target="#fenxiFourNav1FiveNav2Con" data-toggle="tab">历史统计</li>' +
                            '<li data-target="#fenxiFourNav1FiveNav3Con" data-toggle="tab">重点关注工号</li>' +
                            '<li data-target="#fenxiFourNav1FiveNav4Con" data-toggle="tab">重点关注用户</li>');
                        // 排名汇总tab页处于显示状态
                        ...
                        if (data.ygycpmhz) {
                            ...
                            if (data.ygycqtcd) {
                                ...
                                $('#fenxiFourNav1Con .five_nav>ul').append('<li data-target="#fenxiFourNav1FiveNav2Con" data-toggle="tab">历史统计</li>' +
                                    '<li data-target="#fenxiFourNav1FiveNav3Con" data-toggle="tab">重点关注工号</li>' +
                                    '<li data-target="#fenxiFourNav1FiveNav4Con" data-toggle="tab">重点关注用户</li>');
                ...
                if (data.ygycwjxz) {
                    ...
                    if (data.ygycpmhz || data.ygycqtcd) {
                        // 只添加审计报告tab按钮
                        $('#fenxiTabCon .four_nav>ul').append('<li class="col-xs-3" data-target="#fenxiFourNav2Con" data-toggle="tab">审计报告</li>');
                    } else {
                        // 添加审计报告tab按钮-并处于活动状态
                        $('#fenxiTabCon .four_nav>ul').append('<li class="col-xs-3 active" data-target="#fenxiFourNav2Con" data-toggle="tab">审计报告</li>');
                        // 审计报告tab页处于显示状态
                        $('#fenxiFourNav2Con').addClass('active');
                    }
                }
            }
            ...
            if (data.ygycsjjg && (data.ygycpmhz || data.ygycqtcd || data.ygycwjxz)) {
                ...
                // 只有统计分析-有权限
                if (data.ygycpmhz || data.ygycqtcd || data.ygycwjxz) {
                    // 主体右侧显示
...
function initEvent() {
    ...
    $('#provinceList').on('click', 'li a', function () {
        ...
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '区域选择', '查询');
    ...
    $('#timeList').on('click', 'li a', function () {
        ...
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '时间选择', '查询');
        ...
        $(this).closest('.dropdown_menu').slideUp();
        // 改变隐藏域val
        $('#audTrm').val($(this).attr('id').substring());
        // 加载数据
        tabActive();
    });

    //省份地图返回按钮-第一阶段只实现与审计结果的联动
    $("#ProvBackBtn").on('click', function () {
        // 插入一经事件码-查询
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '全国地图', '查询');
    ...
    // 地图-涉及用户对应工号信息表-操作员工号-点击下钻到员工信息
    $('#dialogQudaoTable').on('click', 'a', function () {
        // 插入一经事件码-查询
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', $(this).text() + '员工信息', '查询');
        ...
        $('#TableToInfo').show();
        // 改变隐藏域val
        $('#staffId').val($(this).attr('value'));
        // 加载数据-（第二个下钻表格）
        load_cty_accountIdInfo_table();
    });

    // 地图员工信息表-用户手机号-点击下钻到赠送明细表
    $('#qudaoInfo').on('click', 'a', function () {
        // 插入一经事件码-查询
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', $(this).text() + '赠送明细', '查询');

        // 样式
        $('#TableToInfo').hide();
        $('#TableToInfo1').show();
        // 改变隐藏域val
        $('#staffId').val($(this).parent().prev().text());
        $('#acctId').val($(this).attr('value'));
        // 加载数据-(第三个下钻表格)
        load_cty_totalAmtInfo_table();
    });
    ...
    $('#threeLvNav').on('click', '#resuleBtn', function () {
        ...
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '审计结果', '查询');
    ...
    $('#resultTabCon .four_nav ul').on('click', 'li', function () {
        ...
        // 判断关注点
        switch (concernBtn) {
            case 'abnormalZSHFBtn': //异常高额话费赠送
                //改变隐藏域的val
                $('#concern').val(5003);
                // 改变样式
                $('#contentShowWrap1').siblings().children('p').html('异常赠送金额排名（百万元）');
                $('#contentShowWrap3').siblings().children('p').html('异常赠送金额趋势（百万元）');
                break;
            case 'abnormalHFZYBtn': //异常高额退费
                //改变隐藏域的val
                $('#concern').val(5004);
                // 改变样式
                $('#contentShowWrap1').siblings().children('p').html('异常退费金额排名（百万元）');
                $('#contentShowWrap3').siblings().children('p').html('异常退费金额趋势（百万元）');
                break;
            case 'abnormalZSJFBtn': //异常高额积分赠送
                //改变隐藏域的val
                $('#concern').val(5001);
                // 改变样式
                $('#contentShowWrap1').siblings().children('p').html('异常赠送积分价值金额排名（百万元）');
                $('#contentShowWrap3').siblings().children('p').html('异常赠送积分价值金额趋势（百万元）');
                break;
            case 'abnormalJFZYBtn': //异常高额积分转移
                //改变隐藏域的val
                $('#concern').val(5002);
                // 改变样式
                $('#contentShowWrap1').siblings().children('p').html('异常转移积分价值金额排名（百万元）');
                $('#contentShowWrap3').siblings().children('p').html('异常转移积分价值金额趋势（百万元）');
                break;

        }
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', $(this).text(), '查询');
        // 改变隐藏
        $('#targetTxt').val(concernTitle);
        // 加载地图
        drawMap();
        // 加载审计结果图形
        load_result_chart();
    ...
    $('#threeLvNav').on('click', '#fenxiBtn', function () {
        ...
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '统计分析', '查询');
        ...
        if ($('#fenxiFourNav1FiveNav1Con').hasClass('active')) { //排名汇总有权限
            ...
            load_fenxi_lstj_chart();
        } else if ($('#fenxiFourNav2Con').hasClass('active')) { //审计报告有权限
    ...
    $('#fenxiTabCon .four_nav ul').on('shown.bs.tab', 'li[data-target="#fenxiFourNav1Con"]', function () {
        ...
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '统计报表', '查询');
    ...
    $('#fenxiFourNav1Con .five_nav ul').on('click', 'li', function () {
        ...
        switch (dataText) {
            case '#fenxiFourNav1FiveNav1Con': // 排名汇总
                $("#chooseType").val("异常高额话费赠送");
                load_fenxi_pmhz_table();
                break;
            case '#fenxiFourNav1FiveNav2Con': // 历史统计
                $("#chooseType2").val("异常高额话费赠送");
                load_fenxi_lstj_chart();
                break;
            case '#fenxiFourNav1FiveNav3Con': // 重点关注工号
                $("#chooseType3").val("异常高额话费赠送");
                load_fenxi_zdgz_Num_table();
                break;
            case '#fenxiFourNav1FiveNav4Con': // 重点关注用户
                $("#chooseType4").val("异常高额话费赠送");
    ...
    $('#COUNTForm .dropdown_list').on('click', 'li a', function () {
        ...
        // 加载数据
        switch (objID) {
            case 'chooseType': // 排名汇总
                // 日志记录
                get_userBehavior_log('专题', '员工异常业务操作', $(this).text() + '排名汇总', '查询');
                // 请求数据
                load_fenxi_pmhz_table();
                break;
            case 'chooseType2': // 历史统计
                // 日志记录
                get_userBehavior_log('专题', '员工异常业务操作', $(this).text() + '历史统计', '查询');
                // 请求数据
                load_fenxi_lstj_chart();
                break;
            case 'chooseType3': // 重点关注工号
                // 日志记录
                get_userBehavior_log('专题', '员工异常业务操作', $(this).text() + '重点关注工号', '查询');
                // 请求数据
                load_fenxi_zdgz_Num_table();
                break;
            case 'chooseType4': // 重点关注用户
                // 日志记录
                get_userBehavior_log('专题', '员工异常业务操作', $(this).text() + '重点关注用户', '查询');
                // 请求数据
                load_fenxi_zdgz_User_table();
                break;
        }
    ...
    $('#fenxiTabCon .four_nav ul').on('shown.bs.tab', 'li[data-target="#fenxiFourNav2Con"]', function () {
        ...
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '审计报告', '查询');

        load_fenxi_sjbg_preview();
    });

    // 审计报告下载
    $('#sjbgDownBtn').click(function () {
        // 插入一经事件码-附件下载
        dcs.addEventCode('MAS_HP_CMCA_child_down_file_06');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '审计报告下载', '导出');
        // 请求数据
        down_sjbg_file();
    });

    // 审计清单下载
    $('#sjqdDownBtn').click(function () {
        // 插入一经事件码-附件下载
        dcs.addEventCode('MAS_HP_CMCA_child_down_file_06');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '审计清单下载', '导出');
        // 请求数据
        down_sjqd_file();
    });

    //重点关注工号表格-操作员工标识下钻
    $('#ycghfxTable').on('click', 'a', function () {
        // 插入一经事件码-查询
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '重点关注工号下钻查询', '查询');

        // 样式
        $('#ycghfxTable').closest('.wrap').hide().siblings().show();
        // 改变隐藏域
        $('#staffId').val($(this).attr('value'));
        // 加载数据
        load_fenxi_zdgz_NumInfo_table();
    });

    // 统计分析-重点关注工号下钻返回按钮返回/重点关注工号按钮点击初始化
    $('#ycghfxTableBackBtn').click(function () {
        // 插入一经事件码-查询
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '重点关注工号', '查询');
        // 样式
        $('#ycghfxTableTo').closest('.wrap').hide().siblings().show();
        // 加载数据
        load_fenxi_zdgz_Num_table();
    });

    //重点关注用户表格-操作工号数下钻
    $('#sjyhfxTable').on('click', 'a', function () {
        // 插入一经事件码-查询
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '重点关注用户下钻查询', '查询');

        // 样式
        $('#sjyhfxTable').closest('.wrap').hide().siblings().show();
        // 改变隐藏域val
        $('#acctId').val($(this).attr('value'));
        // 加载数据
        load_fenxi_zdgz_UserInfo_table();
    });

    // 统计分析-重点关注用户下钻返回按钮返回/重点关注用户按钮点击初始化
    $('#sjyhfxTableBackBtn').click(function () {
        // 插入一经事件码-查询
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
        // 日志记录
        get_userBehavior_log('专题', '员工异常业务操作', '重点关注用户', '查询');

        // 样式
        $('#sjyhfxTableTo').closest('.wrap').hide().siblings().show();
        // 加载数据
        load_fenxi_zdgz_User_table();
    });

    //点击中间的左右按钮切换页面布局
    $('#mainLeftBtn').click(function (e) {
        // 插入一经事件码-查询
        dcs.addEventCode('MAS_HP_CMCA_child_query_02');
...
function initDefaultParams() {
    ...
    $.ajax({
        ...
        success: function (data) {
             ...
             */
            $.ajax({
                url: "/cmca/ygyc/getPrvdAndAudTrmInfoData",
                async: false,
                dataType: 'json',
                data: postData,
                success: function (data) {
...
function tabActive() {
    ...
    if ($('#mainRightShow').is(':visible')) {
        ...
        if ($('#resultTabCon').is(':visible')) { //审计结果处于显示状态
            ...
            // step3.判断右侧区域-统计分析-统计报表/审计报告显示状态
            if ($('#fenxiFourNav1Con').is(':visible')) { //统计报表处于显示状态
                // step4.判断右侧区域-统计分析-统计报表-各级tab显示状态
                if ($('#fenxiFourNav1FiveNav1Con').is(':visible')) { //排名汇总
                    load_fenxi_pmhz_table();
                } else if ($('#fenxiFourNav1FiveNav2Con').is(':visible')) { //历史统计
                    load_fenxi_lstj_chart();
                } else if ($('#fenxiFourNav1FiveNav3Con').is(':visible')) {
                    if ($('#FocusNumWrap').is(':visible')) {
                        load_fenxi_zdgz_NumInfo_table(); //重点关注工号分析下钻
                    } else {
                        load_fenxi_zdgz_Num_table(); //重点关注工号
                    }
                } else if ($('#fenxiFourNav1FiveNav4Con').is(':visible')) {
                    if ($('#FocusPerWrap').is(':visible')) {
                        load_fenxi_zdgz_UserInfo_table(); //重点关注用户下钻
                    } else {
                        load_fenxi_zdgz_User_table(); //重点关注用户
                    }
                }
            } else { //审计报告处于显示状态
                load_fenxi_sjbg_preview();
