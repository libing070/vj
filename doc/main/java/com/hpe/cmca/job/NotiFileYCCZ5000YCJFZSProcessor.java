...
@Service("NotiFileYCCZ5000YCJFZSProcessor")
public class NotiFileYCCZ5000YCJFZSProcessor extends AbstractNotiFileProcessor {
	...
	if (this.prvdId.equals("10000")) {
	    this.setFileName("员工异常操作异常积分赠送排名汇总_" + month);
	    // 创建第一个sheet
	    sh = wb.createSheet("表1 异常积分赠送积分值排名前50的用户");
	    // getNotiFile5000YCJFZS()根据月份month、省份prvdId、类型Top50获取后台数据。
	    writeFirstPart(notiFileGenService.getNotiFile5000YCJFZS(month, prvdId, "Top50"), sh, prvdId, month);

	    // 创建第二个sheet
	    sh = wb.createSheet("表2异常积分赠送积分值排名前50用户明细");
	    writeDetailsPart(notiFileGenService.getNotiFile5000YCJFZS(month, prvdId, "Details"), sh, prvdId, month);
	    
	    // 创建第二个sheet
	    sh = wb.createSheet("表3 异常积分赠送积分值公司排名");
	    writeSecondPart(notiFileGenService.getNotiFile5000YCJFZS(month, prvdId, "COMTop50"), sh, prvdId, month);

	    // 创建第三个sheet
	    sh = wb.createSheet("表4 异常积分赠送积分值排名前50用户涉及操作工号");
	    // sh = wb.createSheet("各省疑似养卡号码数量排名前十渠道_" + month);
	    writeThirdPart(notiFileGenService.getNotiFile5000YCJFZS(month, prvdId, "Top50ReleseStaff"), sh, prvdId, month);

	    // 创建第四个sheet
	    sh = wb.createSheet("表5 异常积分赠送排名前50操作工号");
	    writeFourthPart(notiFileGenService.getNotiFile5000YCJFZS(month, prvdId, "StaffTop50"), sh, prvdId, month);

	} else {
	    this.setFileName("员工异常操作异常积分赠送排名汇总_" + prvdName + "_" + month);
	    // 创建第一个sheet
	    sh = wb.createSheet("表1 异常积分赠送积分值排名前50的用户");
	    // getNotiFile5000YCJFZS()根据月份month、省份prvdId、类型Top50获取后台数据。
	    writeFirstPart(notiFileGenService.getNotiFile5000YCJFZS(month, prvdId, "Top50"), sh, prvdId, month);
	    ...
	    // 创建第二个sheet
	    sh = wb.createSheet("表2 异常积分赠送积分值公司排名");
	    writeSecondPart(notiFileGenService.getNotiFile5000YCJFZS(month, prvdId, "COMTop50"), sh, prvdId, month);

	    // 创建第三个sheet
	    sh = wb.createSheet("表3 异常积分赠送积分值排名前50用户涉及操作工号");
	    // sh = wb.createSheet("各省疑似养卡号码数量排名前十渠道_" + month);
	    writeThirdPart(notiFileGenService.getNotiFile5000YCJFZS(month, prvdId, "Top50ReleseStaff"), sh, prvdId, month);

	    // 创建第四个sheet
	    sh = wb.createSheet("表4 异常积分赠送排名前50操作工号");
	    writeFourthPart(notiFileGenService.getNotiFile5000YCJFZS(month, prvdId, "StaffTop50"), sh, prvdId, month);
     ...
     * 
     * <pre>
     * Desc  将获取到的数据插入到excel‘员工异常操作异常积分赠送排名汇总’下的sheet1中
     * @param data：获取的数据
     * @param sh：应插入的sheet
     * @param prvdId：省份
     * @param month：月份
	...
	sh.createRow(0);
	sh.getRow(0).createCell(0).setCellValue("表1 异常积分赠送积分值排名前50的用户");
	sh.createRow(1);
	sh.getRow(1).createCell(0).setCellValue("审计月");
	sh.getRow(1).createCell(1).setCellValue("赠送积分值全国排序");
	sh.getRow(1).createCell(2).setCellValue("赠送积分值省份排序");
	sh.getRow(1).createCell(3).setCellValue("省份");
	sh.getRow(1).createCell(4).setCellValue("地市");
	sh.getRow(1).createCell(5).setCellValue("赠送积分值");
	sh.getRow(1).createCell(6).setCellValue("赠送积分值价值金额（元）");
	sh.getRow(1).createCell(7).setCellValue("赠送积分值价值金额（万元）");
	sh.getRow(1).createCell(8).setCellValue("获赠用户标识");
	sh.getRow(1).createCell(9).setCellValue("获赠手机号");
	sh.getRow(1).createCell(10).setCellValue("赠送次数");
	...
	sh.getRow(1).createCell(15).setCellValue("客户标识");
	sh.getRow(1).createCell(16).setCellValue("客户欠费金额（元）");
	sh.getRow(1).createCell(17).setCellValue("欠费账龄");
	sh.getRow(1).createCell(18).setCellValue("欠费月数");
	sh.getRow(1).createCell(19).setCellValue("T-1月出账金额（元）");
	sh.getRow(1).createCell(20).setCellValue("T-2月出账金额（元）");
	sh.getRow(1).createCell(21).setCellValue("T-3月出账金额（元）");
	...
	for (int i = 0, dataSize = data.size(); i < dataSize; i++) {
		...
		if (row.get("cust_id") != null) {
		    sh.getRow(2 + i).createCell(15).setCellValue(row.get("cust_id").toString());
		} else {
		    sh.getRow(2 + i).createCell(15).setCellValue("");
		}
		if (row.get("sum_debt_amt") != null) {
		    sh.getRow(2 + i).createCell(16).setCellValue(
		    		row.get("sum_debt_amt") instanceof BigDecimal ? ((BigDecimal) row.get("sum_debt_amt")).doubleValue() : (Long) row.get("sum_debt_amt"));
		} else {
		    sh.getRow(2 + i).createCell(16).setCellValue("");
		}
		//员工异常操作2.0 新增欠费账龄、欠费月数
		if (row.get("aging") != null) {
		    sh.getRow(2 + i).createCell(17).setCellValue((Integer) row.get("aging"));
		} else {
		    sh.getRow(2 + i).createCell(17).setCellValue("");
		}
		if (row.get("dbt_mons") != null) {
		    sh.getRow(2 + i).createCell(18).setCellValue((Integer) row.get("dbt_mons"));
		} else {
		...
		if (row.get("out_amt_t1") != null) {
		    sh.getRow(2 + i).createCell(19).setCellValue(
		    		row.get("out_amt_t1") instanceof BigDecimal ? ((BigDecimal) row.get("out_amt_t1")).doubleValue() : (Long) row.get("out_amt_t1"));
		} else {
		    sh.getRow(2 + i).createCell(19).setCellValue("-");
		}
		if (row.get("out_amt_t2") != null) {
		    sh.getRow(2 + i).createCell(20).setCellValue(
		    		row.get("out_amt_t2") instanceof BigDecimal ? ((BigDecimal) row.get("out_amt_t2")).doubleValue() : (Long) row.get("out_amt_t2"));
		} else {
		    sh.getRow(2 + i).createCell(20).setCellValue("-");
		}
		if (row.get("out_amt_t3") != null) {
		    sh.getRow(2 + i).createCell(21).setCellValue(
		    		row.get("out_amt_t3") instanceof BigDecimal ? ((BigDecimal) row.get("out_amt_t3")).doubleValue() : (Long) row.get("out_amt_t3"));
		} else {
		    sh.getRow(2 + i).createCell(21).setCellValue("-");
		}
	    }
	    // 设置单元格格式
	    // 赠送积分值价值金额（元）、赠送积分值价值金额（万元）格式保留两位小数
	...
	sh.createRow(0);
	if(prvdId.equals("10000")){
	    sh.getRow(0).createCell(0).setCellValue("表3 异常积分赠送积分值公司排名");
	}else{
	    sh.getRow(0).createCell(0).setCellValue("表2 异常积分赠送积分值公司排名");
	}
	sh.createRow(1);
	sh.getRow(1).createCell(0).setCellValue("审计月");
	// if(prvdId.equals("10000")){
	sh.getRow(1).createCell(1).setCellValue("赠送积分值排名");
	sh.getRow(1).createCell(2).setCellValue("省份");
	sh.getRow(1).createCell(3).setCellValue("违规地市数量");
	sh.getRow(1).createCell(4).setCellValue("赠送积分值");
	sh.getRow(1).createCell(5).setCellValue("赠送积分值价值金额（元）");
	sh.getRow(1).createCell(6).setCellValue("赠送积分值价值金额（万元）");
	...
	sh.createRow(0);
	if(prvdId.equals("10000")){
	    sh.getRow(0).createCell(0).setCellValue("表4 异常积分赠送积分值排名前50用户涉及操作工号");
	}else{
	    sh.getRow(0).createCell(0).setCellValue("表3 异常积分赠送积分值排名前50用户涉及操作工号"); 
	}
	...
	sh.getRow(1).createCell(9).setCellValue("赠送次数");
	...
	sh.createRow(0);
	if(prvdId.equals("10000")){
	    sh.getRow(0).createCell(0).setCellValue("表5 异常积分赠送积分值排名前50操作工号");
	}else{
	    sh.getRow(0).createCell(0).setCellValue("表4 异常积分赠送积分值排名前50操作工号"); 
	}

	sh.createRow(1);
	sh.getRow(1).createCell(0).setCellValue("审计月");
	sh.getRow(1).createCell(1).setCellValue("赠送积分值排序");
	sh.getRow(1).createCell(2).setCellValue("省份");
	sh.getRow(1).createCell(3).setCellValue("地市");
	sh.getRow(1).createCell(4).setCellValue("赠送积分值");
	sh.getRow(1).createCell(5).setCellValue("赠送积分对应价值金额（元）");
	sh.getRow(1).createCell(6).setCellValue("赠送积分对应价值金额（万元）");
	...
	sh.createRow(0);
	if(prvdId.equals("10000")){
	    sh.getRow(0).createCell(0).setCellValue("表2 异常积分赠送积分值排名前50用户明细");
	}
	sh.createRow(1);
	sh.getRow(1).createCell(0).setCellValue("审计月");
	...
	sh.getRow(1).createCell(6).setCellValue("交易积分值");
	sh.getRow(1).createCell(7).setCellValue("手机号码");
	...
	sh.getRow(1).createCell(11).setCellValue("积分服务渠道标识");
	sh.getRow(1).createCell(12).setCellValue("积分服务渠道名称");
	sh.getRow(1).createCell(13).setCellValue("操作员工归属渠道标识");
	sh.getRow(1).createCell(14).setCellValue("操作员工归属渠道名称");
	sh.getRow(1).createCell(15).setCellValue("操作员工归属渠道类型");
	sh.getRow(1).createCell(16).setCellValue("业务交易类型");
	sh.getRow(1).createCell(17).setCellValue("业务类型说明");
	sh.getRow(1).createCell(18).setCellValue("积分类型");
	sh.getRow(1).createCell(19).setCellValue("积分类型说明");
	sh.getRow(1).createCell(20).setCellValue("积分变动原因");
	sh.getRow(1).createCell(21).setCellValue("违规类型标识");
	sh.getRow(1).createCell(22).setCellValue("违规类型说明");
	sh.getRow(1).createCell(23).setCellValue("客户标识");
	sh.getRow(1).createCell(24).setCellValue("客户欠费金额（元）");
	// 设置第一行单元格格式 及 合并单元格 -add by hufei
	sh.getRow(0).getCell(0).setCellStyle(mapStyle.get("style0"));
	sh.addMergedRegion(new CellRangeAddress(0, 0, 0, 24));
	...
	for (int i = 0, dataSize = data.size(); i < dataSize; i++) {
	    ...
	    sh.createRow(2 + i);
	    for(int m=0;m<=24;m++){
		sh.getRow(2 + i).createCell(m).setCellStyle(mapStyle.get("style3"));
	    }
	    // 插入单元格数据
	    if (row.size() == 0) {
		    ...
		    if (row.get("Aud_trm") != null) {
			sh.getRow(2 + i).getCell(0).setCellValue(row.get("Aud_trm").toString());
		    } else {
			sh.getRow(2 + i).getCell(0).setCellValue("");
		    }
		    if (row.get("prvd_id") != null) {
			sh.getRow(2 + i).getCell(1).setCellValue(row.get("prvd_id").toString());
		    } else {
			sh.getRow(2 + i).getCell(1).setCellValue("");
		    }
		    if (row.get("prvd_name") != null) {
			sh.getRow(2 + i).getCell(2).setCellValue(row.get("prvd_name").toString());
		    } else {
			sh.getRow(2 + i).getCell(2).setCellValue("");
		    }
		    if (row.get("city_id") != null) {
			sh.getRow(2 + i).getCell(3).setCellValue(row.get("city_id").toString());
		    } else {
			sh.getRow(2 + i).getCell(3).setCellValue("");
		    }
		    if (row.get("city_name") != null) {
			sh.getRow(2 + i).getCell(4).setCellValue(row.get("city_name").toString());
		    } else {
			sh.getRow(2 + i).getCell(4).setCellValue("");
		    }
		    if (row.get("trade_ser_no") != null) {
			sh.getRow(2 + i).getCell(5).setCellValue(row.get("trade_ser_no").toString());
		    } else {
			sh.getRow(2 + i).getCell(5).setCellValue("");
		    }
		    if (row.get("trade_value") != null) {
			sh.getRow(2 + i).getCell(6)
				.setCellValue(row.get("trade_value") instanceof BigDecimal ? ((BigDecimal) row.get("trade_value")).doubleValue() : (Long) row.get("total_Amt_10000"));
		    } else {
			sh.getRow(2 + i).getCell(6).setCellValue("");
		    }
		    if (row.get("msisdn") != null) {
			sh.getRow(2 + i).getCell(7).setCellValue(row.get("msisdn").toString());
		    } else {
			sh.getRow(2 + i).getCell(7).setCellValue("");
		    }
		    if (row.get("subs_id") != null) {
			sh.getRow(2 + i).getCell(8).setCellValue(row.get("subs_id").toString());
		    } else {
			sh.getRow(2 + i).getCell(8).setCellValue("");
		    }
		    if (row.get("trade_tm") != null) {
			sh.getRow(2 + i).getCell(9).setCellValue(row.get("trade_tm").toString());
		    } else {
			sh.getRow(2 + i).getCell(9).setCellValue("");
		    }
		    if (row.get("staff_id") != null) {
			sh.getRow(2 + i).getCell(10).setCellValue(row.get("staff_id").toString());
		    } else {
			sh.getRow(2 + i).getCell(10).setCellValue("");
		    }
		    if (row.get("points_chnl_id") != null) {
			sh.getRow(2 + i).getCell(11).setCellValue(row.get("points_chnl_id").toString());
		    } else {
			sh.getRow(2 + i).getCell(11).setCellValue("");
		    }
		    if (row.get("points_chnl_nm") != null) {
			sh.getRow(2 + i).getCell(12).setCellValue(row.get("points_chnl_nm").toString());
		    } else {
			sh.getRow(2 + i).getCell(12).setCellValue("");
		    }
		    if (row.get("blto_chnl_id") != null) {
			sh.getRow(2 + i).getCell(13).setCellValue(row.get("blto_chnl_id").toString());
		    } else {
			sh.getRow(2 + i).getCell(13).setCellValue("");
		    }
		    if (row.get("chnl_nm") != null) {
			sh.getRow(2 + i).getCell(14).setCellValue(row.get("chnl_nm").toString());
		    } else {
			sh.getRow(2 + i).getCell(14).setCellValue("");
		    }
		    if (row.get("blto_chnl_typ") != null) {
			sh.getRow(2 + i).getCell(15).setCellValue(row.get("blto_chnl_typ").toString());
		    } else {
			sh.getRow(2 + i).getCell(15).setCellValue("");
		    }
		    if (row.get("trade_typ") != null) {
			sh.getRow(2 + i).getCell(16).setCellValue(row.get("trade_typ").toString());
		    } else {
			sh.getRow(2 + i).getCell(16).setCellValue("");
		    }
		    if (row.get("trade_typ_nm") != null) {
			sh.getRow(2 + i).getCell(17).setCellValue(row.get("trade_typ_nm").toString());
		    } else {
			sh.getRow(2 + i).getCell(17).setCellValue("");
		    }
		    if (row.get("points_typ") != null) {
			sh.getRow(2 + i).getCell(18).setCellValue(row.get("points_typ").toString());
		    } else {
			sh.getRow(2 + i).getCell(18).setCellValue("");
		    }
		    if (row.get("points_typ_nm") != null) {
			sh.getRow(2 + i).getCell(19).setCellValue(row.get("points_typ_nm").toString());
		    } else {
			sh.getRow(2 + i).getCell(19).setCellValue("");
		    }
		    if (row.get("points_change_reason") != null) {
			sh.getRow(2 + i).getCell(20).setCellValue(row.get("points_change_reason").toString());
		    } else {
			sh.getRow(2 + i).getCell(20).setCellValue("");
		    }
		    if (row.get("infrac_typ") != null) {
			sh.getRow(2 + i).getCell(21).setCellValue(row.get("infrac_typ").toString());
		    } else {
			sh.getRow(2 + i).getCell(21).setCellValue("");
		    }
		    if (row.get("infrac_typ_nm") != null) {
			sh.getRow(2 + i).getCell(22).setCellValue(row.get("infrac_typ_nm").toString());
		    } else {
			sh.getRow(2 + i).getCell(22).setCellValue("");
		    }
		    if (row.get("cust_id") != null) {
			sh.getRow(2 + i).getCell(23).setCellValue(row.get("cust_id").toString());
		    } else {
			sh.getRow(2 + i).getCell(23).setCellValue("-");
		    }
		    if (row.get("sum_debt_amt") != null) {
			sh.getRow(2 + i).getCell(24).setCellValue(
			    		row.get("sum_debt_amt") instanceof BigDecimal ? ((BigDecimal) row.get("sum_debt_amt")).doubleValue() : (Long) row.get("sum_debt_amt"));
		    } else {
			sh.getRow(2 + i).getCell(24).setCellValue("-");
		    }
	    // 设置单元格格式
	    // 赠送积分值价值金额（元）格式保留两位小数
