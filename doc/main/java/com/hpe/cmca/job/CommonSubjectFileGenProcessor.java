...
public class CommonSubjectFileGenProcessor extends BaseObject implements IFileGenProcessor {
	...
	try {
	    ...
	    concernFileGenService.updateFileRequestExecCountBysubject(request);// 按专题更新执行次数
	}
    }
	...
	this.logger.info("该专题汇总关注点(员工异常操作是各个子关注点)：是" + focusCd);
	if ("5".equals(subjectId)||"6".equals(subjectId)||"1".equals(subjectId)) {
	    configInfo = concernFileGenService.selectFileConfig(subjectId, focusCd);
	}
	String sql = (String) configInfo.get("csvSql");
	File file = new File(FileUtil.buildFullFilePath(localFilePath, localFileName));
	...
	try {
	    ...
	    if(csvFile != null){
        	    ...
        	    updateFileRequestStatusBySubjectId(modelNotifyId, audTrm, subjectId, totalFocusCds, prvdId, request, Constants.Model.FileRequestStatus.CSV_FILE_FINISHED);
        	    ...
        	    updateFileRequestStatusBySubjectId(modelNotifyId, audTrm, subjectId, totalFocusCds, prvdId, request, Constants.Model.FileRequestStatus.CSV_FTP_FINISHED);

        	    // 向文件生成结果表HPMGR.busi_report_file中插入新生成的文件信息
        	    insertReportFile(modelNotifyId, audTrm, subjectId, totalFocusCds, prvdId, Constants.Model.FileType.AUD_DETAIL, csvFtpPutResult);
	    }
	    if (flag) {
		...
		// updateFileRequestStatus(modelNotifyId, audTrm, subjectId, totalFocusCds, prvdId, request, Constants.Model.FileRequestStatus.DOC_FILE_FINISHED);
		updateFileRequestStatusBySubjectId(modelNotifyId, audTrm, subjectId, totalFocusCds, prvdId, request, Constants.Model.FileRequestStatus.DOC_FILE_FINISHED);

		// 上传Doc.zip，更新数据库请求记录状态
		Map<String, Object> docFtpPutResult = transferFileToFtpServer(Constants.Model.FileType.AUD_REPORT, audTrm, subjectId, totalFocusCds, prvdId, docFile, useChineseName);
		...
		// 向文件生成结果表HPMGR.busi_report_file中插入新生成的文件信息
		insertReportFile(modelNotifyId, audTrm, subjectId, totalFocusCds, prvdId, Constants.Model.FileType.AUD_REPORT, docFtpPutResult);
	    }
	    ...
	    updateFileRequestStatusBySubjectId(modelNotifyId, audTrm, subjectId, totalFocusCds, prvdId, request, Constants.Model.FileRequestStatus.File_FINISHED);
	...
	try {
	    ...
	    updateFileRequestStatusBySubjectId(modelNotifyId, audTrm, subjectId, focusCd, prvdId, request, Constants.Model.FileRequestStatus.DOC_FILE_FINISHED);
	    // 第二步：从数据库中查询已生成的32份省级审计报告文件名，进行打包
	    String localFilePath = getLocalFilePath();
	    List<String> fileDocList = concernFileGenService.selectAuditResultFile(audTrm, subjectId, focusCd, Constants.Model.FileType.AUD_REPORT);
	    fileDocList.add(docFile.getName());// 增加全公司Doc文件
	    String docZipFileName = buildFileName(Constants.Model.FileType.AUD_REPORT, audTrm, subjectId, focusCd, prvdId, useChineseName);
	    ...
	    List<String> fileCsvList = concernFileGenService.selectAuditResultFile(audTrm, subjectId, focusCd, Constants.Model.FileType.AUD_DETAIL);
	    ...
	    Map<String, Object> docFtpPutResult = transferFileToFtpServer(Constants.Model.FileType.AUD_REPORT, audTrm, subjectId, focusCd, Constants.ChinaCode, docZipFile, useChineseName);
	    docFtpPutResult.put("createTime", request.get("docFileGenTime"));
	    request.put("docFileFtpTime", new Date());
	    // updateFileRequestStatus(modelNotifyId, audTrm, subjectId, focusCd, prvdId, request, Constants.Model.FileRequestStatus.DOC_FTP_FINISHED);
	    updateFileRequestStatusBySubjectId(modelNotifyId, audTrm, subjectId, focusCd, prvdId, request, Constants.Model.FileRequestStatus.DOC_FTP_FINISHED);

	    // 第五步：删除HPMGR.busi_report_file表中原文件，插入新生成的csv文件记录；
	    insertReportFile(modelNotifyId, audTrm, subjectId, focusCd, prvdId, Constants.Model.FileType.AUD_DETAIL, csvFtpPutResult);
	    // 第六步：删除HPMGR.busi_report_file表中原文件，插入新生成的doc文件记录；
	    insertReportFile(modelNotifyId, audTrm, subjectId, focusCd, prvdId, Constants.Model.FileType.AUD_REPORT, docFtpPutResult);
     ...
     */
    @Transactional(propagation = Propagation.REQUIRED)
    public void insertReportFile(int modelNotifyId, String audTrm, String subjectId, String focusCd, int chinacode, String fileType, Map<String, Object> ftpPutResult) {
	...
	concernFileGenService.insertReportFile(modelNotifyId, audTrm, subjectId, focusCd, chinacode, fileType, filePath, fileName, downloadUrl, createTime);
    }
    ...
    private Map<String, Object> transferFileToFtpServer(String fileType, String audTrm, String subjectId, String focusCd, int prvdId, File upFile, Boolean useChineseName) throws Exception {
	...
	if (useChineseName) {
	    ...
	    if ("5".equals(subjectId) && Constants.Model.FileType.AUD_DETAIL.equalsIgnoreCase(fileType)) {
    ...
    */
	...
	if (useChineseName) {
	    // 生成中文名审计报告/csv
	    prvdName = Constants.MAP_PROVD_NAME.get(prvdId);
	    if (("1".equals(subjectId)||"5".equals(subjectId)||"7".equals(subjectId)||"6".equals(subjectId)) && Constants.Model.FileType.AUD_DETAIL.equalsIgnoreCase(fileType)) {
		String focusName = Constants.MAP_FOCUSCD_NAME.get(focusCd);
		path.append(prvdName).append("_").append(audTrm).append("_").append(focusName);
	    } else {
		subjectId = Constants.MAP_SUBJECT_NAME.get(subjectId);
	...
	if (Constants.Model.FileType.AUD_DETAIL.equalsIgnoreCase(fileType)) {
	    if (useChineseName && !"7".equals(subjectId)) {
		path.append("审计清单.csv");
	    } else {
		path.append(".csv");
	    }
