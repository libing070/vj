...
public class PageInterceptor implements Interceptor {
	...
	private String getTeraDataPageSql(Pager page, StringBuffer sqlBuffer) {
		...
		if (StringUtils.isBlank(page.getOrderBy())) {
			sqlBuffer
					.insert(0,
							"select u.*, row_number() OVER(ORDER BY 1 desc) RN from (")
					.append(") u QUALIFY  RN< ")
					.append(offset + page.getPageSize()).append(" AND RN>=")
					.append(offset);
		}
		if (StringUtils.isNotBlank(page.getOrderBy())) {
			String sqlPix = "select u.*, row_number() OVER(ORDER BY "
					+ page.getOrderBy() + " asc) RN from (";
			if (page.getOrderDirection() != null) {
				sqlPix = "select u.*, row_number() OVER(ORDER BY "
						+ page.getOrderBy() + " " + page.getOrderDirection()
						+ ") RN from (";
			}
			sqlBuffer.insert(0, sqlPix).append(") u QUALIFY  RN< ")
					.append(offset + page.getPageSize()).append(" AND RN>=")
					.append(offset);
		}
	 ...
	 *            当前的数据库连接
	 */
	private void setTotalRecord(Pager page, MappedStatement mappedStatement,
