(function($) {
	$.fn.extend({
		timeStyle:function(time){
			var year = time.substring(0,4);
			var mouth = time.substring(4,6);
			return year+"-"+ mouth;
			
		},
		startMonthThanEndMonth : function() {
		    return "您选择的时间,结束时间应该不小于开始时间！";
		},
		ctx : function() {
			//return "/prx/000/http/10.1.26.5:8888/eap";
			return "/cmwa";
		},
		cmwaurl : function() {
			//return "https://bass.hq.cmcc/prx/000/http/10.1.26.5:8888/eap";
			//return "/prx/000/http/10.1.26.5:8888/eap";		
			return "/cmwa";
		},
		noRecordMsg : function() {
			return "没有符合条件的数据！";
		},
		loadIframePage : function(frameName, url) {
			$("#" + frameName).attr("src", $.fn.ctx() + url);
			$("#" + frameName).load(function() {
				var mainheight = $(this).contents().find("body").height() + 30;
				$(this).height(mainheight);
			});
		},
		resetFrameHeight : function(iframeId) {
		    var iframeId = $("#" + iframeId, window.parent.document);
		    var mainheight = iframeId.contents().find("body").height() + 30;
		    iframeId.height(mainheight);
        },
        getAlarmStatusName : function(code) {
            return ['情况良好', '没有数据', '低风险预警', '高风险预警'][code];
        },
        getConcernTypeName : function(code) {
            return ['经责', '持续审计', '持续审计现场核查'][code];
        },
        tableFieldName : function() {
            return ['BEIJING', 'SHANGHAI', 'TIANJIN', 'CHONGQING', 'GUIZHOU', 'HUBEI', 'SHANXI', 'HEBEI', 'HENAN', 'ANHUI', 'FUJIAN', 'QINGHAI', 'GANSU', 'ZHEJIANG', 'HAINAN', 'HEILONGJIANG', 'JIANGSU', 'JILIN', 'NINGXIA', 'SHANDONG', 'SHANXI2', 'XINJIANG', 'GUANGDONG', 'LIAONING', 'GUANGXI', 'HUNAN', 'JIANGXI', 'NEIMENGGU', 'YUNNAN', 'SICHUAN', 'XIZANG'];
        },
        formatDateMonthCN : function(month) {
            if (month == null || month == "") {
                return "";
            }
		...
		getProvInfo : function(val) {
		    var retObj;
		    var provList = $.fn.provList();
	        $.each(provList, function(idx, obj) {
	            if (obj.provId == val || obj.provName == val || obj.fieldName == val) {
	                retObj = obj;
	                return false;
	            }
	        });
		...
		colorChinaRangeAxis : function(Xvalue,Yvalue) {
			var cs = [];
			cs[3] = {
					color : "#65d3e3",
					from : 0,
					to : Xvalue,
					name : "异常欠费金额比例 < Xvalue"
				};
			cs[2] = {
				color : "#dddddd",
				from : -1,
			...
			cs[1] = {
				...
				from : Xvalue,
				to : Yvalue,
				name : "异常欠费金额比例in [Xvalue,Yvalue]"
			};
			cs[0] = {
				color : "#ff7979",
				from : Yvalue,
				to : 99999,
				name : "异常欠费金额比例 > Yvalue"
			};
			return cs;
		},
		colorRealSumCityAxis : function(lev) {
			var cs = [];
			cs[2] = {
				color : "#dddddd",
				from : -1,
		...
		colorBaremetalCityAxis : function(XNumber,YNumber) {
			var cs = [];
			cs[2] = {
				color : "#dddddd",
				from : -1,
			...
			cs[3] = {
				...
				from : 0,
				to : XNumber,
				name : "违规终端数量占比<"+XNumber
			};
			cs[1] = {
				color : "#65d3e3",
				from : XNumber,
				to : YNumber,
				name : "违规终端数量占比"+XNumber+"-"+YNumber+"之间"
			};
			cs[0] = {
				color : "#f2ca68",
				from : YNumber,
				to : 99999,
				name : "违规终端数量占比>"+YNumber
			};
			return cs;
		},
		colorzdbt7002CityAxis : function(XNumber,YNumber) {
			var cs = [];
			cs[2] = {
				color : "#dddddd",
				from : -1,
			...
			cs[3] = {
				...
				from : 0,
				to : XNumber,
				name : "异常终端IMEI数占比<"+XNumber
			};
			cs[1] = {
				color : "#65d3e3",
				from : XNumber,
				to : YNumber,
				name : "异常终端IMEI数占比"+XNumber+"-"+YNumber+"之间"
			};
			cs[0] = {
				color : "#f2ca68",
				from : YNumber,
				to : 99999,
				name : "异常终端IMEI数占比>"+YNumber
			};
			return cs;
		},
		colorZHBTL7001CityAxis : function(XNumber,YNumber) {
			var cs = [];
			cs[2] = {
				color : "#dddddd",
				from : -1,
			...
			cs[3] = {
				...
				from : 0,
				to : XNumber,
				name : "终端IMEI数<"+XNumber
			};
			cs[1] = {
				color : "#65d3e3",
				from : XNumber,
				to : YNumber,
				name : "终端IMEI数"+XNumber+"-"+YNumber+"之间"
			};
			cs[0] = {
				color : "#f2ca68",
				from : YNumber,
				to : 99999,
				name : "终端IMEI数>"+YNumber
			};
			return cs;
		},
		colorCardSumCityAxis : function(lev) {
			var cs = [];
			cs[2] = {
				color : "#dddddd",
				from : -1,
		...
		riskDomainTable : function() {
			return [ {
				"CAIWU" : "财务管理机制",
				"SHICHANG" : "4_社会渠道和酬金管理",
				"KEHU" : "6_客户信息安全管理",
				"WANGLUO" : "网络规划管理",
				"IT" : "信息技术战略规划管理",
				"ZHINENG" : "人力资源管理"
		...
		base64encode : function(str) {
			...
			var out = $.base64.btoa(str);
			return out;
		},
		// base64解码
		base64decode : function(str) {
			$.base64.utf8encode = true;
			var out = $.base64.atob(str);
			return out;
		},
		chinaMapData : function() {
			...
			return [
					...
					{
						"path" : [ "M", 7477, -2683, "L", 7484, -2676, 7473, -2673, 7469, -2681, 7477, -2683, "M", 7234, -2879, "L", 7234, -2858, 7201, -2854, 7200, -2871, 7234, -2879, "M", 7523, -3257, "L", 7529, -3258, 7521, -3253, 7519, -3256, 7523, -3257, "M", 7875, -3008,
								"L", 7842, -3038, 7862, -3059, 7882, -3019, 7875, -3008, "M", 7638, -2670, "L", 7651, -2672, 7663, -2661, 7652, -2656, 7638, -2670, "M", 7652, -2442, "L", 7615, -2480, 7598, -2540, 7651, -2543, 7695, -2610, 7705, -2625, 7756, -2667, 7764, -2665,
								7768, -2659, 7763, -2643, 7776, -2629, 7750, -2615, 7740, -2529, 7661, -2521, 7652, -2442, "M", 7726, -2950, "L", 7732, -2966, 7733, -2974, 7760, -2958, 7744, -2944, 7740, -2942, 7726, -2950, "M", 7744, -2944, "L", 7760, -2958, 7733, -2974, 7741,
								-2986, 7744, -2996, 7813, -2957, 7837, -2921, 7832, -2904, 7819, -2885, 7817, -2885, 7808, -2883, 7768, -2897, 7744, -2944, "M", 7887, -3052, "L", 7890, -3049, 7902, -3046, 7905, -3030, 7887, -3052, "M", 7744, -2944, "L", 7768, -2897, 7808, -2883,
								-699, -7387, -640, -7355, -561, -7377, -563, -7300, -455, -7310, -448, -7285, -378, -7337, -335, -7404, -288, -7417, -209, -7384, -100, -7373, -32, -7440, 155, -7499, 237, -7503, 260, -7528, 320, -7520, 346, -7568, 343, -7663, 367, -7699, 429,
								-7713, 403, -7750, 481, -7758, 485, -7840, 454, -8006, 484, -8137, 509, -8153, 429, -8185, 412, -8216, 452, -8236, 593, -8233, 754, -8253, 781, -8199, 844, -8207, 891, -8185, 909, -8246, 858, -8281, 937, -8433, 1044, -8601, 1061, -8662, 1084,
								-8662, 1200, -8579, 1328, -8558, 1338, -8513, 1422, -8553, 1472, -8549, 1516, -8625, 1513, -8763, 1580, -8877, 1648, -8867, 1715, -8884, 1753, -8923, 1762, -8995, 1794, -9020, 1877, -8997, 1950, -9009, 1933, -8917, 1971, -8881, 1948, -8863, 2041,
								-8782, 2038, -8749, 2108, -8681, 2192, -8678, 2219, -8621, 2271, -8627, 2313, -8565, 2321, -8474, 2370, -8384, 2392, -8276, 2357, -8215, 2365, -8143, 2306, -8084, 2286, -8023, 2313, -7942, 2359, -7938, 2445, -7892, 2505, -7887, 2583, -7857, 2633,
								-7861, 2763, -7822, 2824, -7745, 2870, -7735, 2891, -7697, 3005, -7620, 3066, -7621, 3048, -7554, 3080, -7544, 3128, -7351, 3189, -7276, 3196, -7224, 3133, -7177, 3118, -7056, 3066, -7022, 2938, -7027, 2847, -6989, 2731, -6888, 2638, -6776, 2582,
								-6755, 2520, -6764, 2505, -6681, 2469, -6612, 2490, -6524, 2478, -6455, "Z" ],
						"name" : "Xinjiang",
						"properties" : {
							"id" : "12200",
							"hc-group" : "admin1",
		...
		setDateTimePicker : function(id, month) {
		    if (typeof(month) == "undefined" || month == null) {
	            // 获取系统当前月
	            $.post($.fn.eapurl() + "/base/getCurrentDate", {}, function(result) {
	                $('#' + id).datepicker('setDate', new Date(result.substring(0, 4), result.substring(4, 6) - 1));
	                $('#' + id).val(result.substring(0, 6));
	            });
	        }
	        else {
	            $('#' + id).datepicker('setDate', new Date(month.substring(0, 4), month.substring(4, 6) - 1));
	            $('#' + id).val(month);
	        }
		},
		
		// 固定表头  放到数据返回成功之后
		setFixHeaderAndNiceScroll : function(layerId, tableId) {
		    jQuery.fn.CloneTableHeader(tableId, layerId);
	        $("#" + layerId).niceScroll({
	            cursorcolor : '#d1d1d1',
	            cursorwidth : '4'
	        });
		...
		// 设置表格最大显示列
    	setTableMaxCols : function(tableId, maxColNum) {
            var cloneTableId = "tableHeaderDiv" + tableId;
            var tdLength = $("#" + tableId + " tbody tr, #" + cloneTableId + " tbody tr").length
            for (var i = 0; i < maxColNum; i++) {
                $("#" + tableId + " tr th, #" + cloneTableId + " tr th").eq(i).show();
                for (var k = 0; k < tdLength; k++) {
                    $("#" + tableId + " tbody tr, #" + cloneTableId + " tbody tr").eq(k).children('td').eq(i).show();
                }
            }
    	}		
	}) // $.fn.extend
...
//根据选择省用户所在省份ID查询地市列表，并初始化 20160921 add by GuoXY 
function queryCityList(prvdId){
	$.ajax({
		url : $.fn.eapurl() + "/base/getCityList",
		async : false,
		dataType : 'json',
		data : {prvdId:prvdId},
		crossDomain : true,
		success : function(data) {
	        // 遍历每一个地市记录，获取其 简名和ID
	        var len = data.body.ctyList.length;
	        var dlCount = Math.ceil(len/3);
	        var ctyList = data.body.ctyList;
	        var str='';
	        var start=0;
	        for(var i=0;i<3;i++){
	        	str+='<dl>';
	        	for(;start<dlCount*(i+1);start++){
	        		if(start<len){
	        			str+='<dd><p><input value="'+ctyList[start].CityCode+'" type="checkbox"><em></em></p><span>'+ctyList[start].name+'</span></dd>';
	        		}
	        	}
	        	str+='</dl>';
	        }
	        $("#cityDivList").html(str);	
		}
	});
}
 ...
 * @param isSaveDec 是否保留小数位(false，用于数量类业务的千分位格式化)
 * @return 金额格式的字符串,如'1,234,567.45'  
 * @type String  
 * 20160929 add by GuoXY 
 */


function formatCurrency(num, isSaveDec) {
 ...
 return (((sign)?'':'-') + num + (isSaveDec ? '.' + cents : ''));  
}  
...
function formatCurrencySjjk(num, isSaveDec) {
	if (typeof(num) == "undefined" || num == null) {
		return "";
	}else{
		if(isSaveDec){
			var res= num.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
			var point= res.split(".");
			if(point.length==1){
				return (res+=".00");
			}
			if(point.length==2){
				return (point[1].length==1)?(res+="0"):res;
				
			}
		}else{
			return toThousands(num.toFixed(0));
		}
	}
}   
...
// 小数保留两位（eg：changeTwoDecimal(3.1415926) 返回 3.14）  20160923 add by GuoXY 
...
//判断金额是否是整数
function AmtIntTwoDecimal(x){
	var ex = /^\d+$/;
	if (ex.test(x)) {
	   return Number(getFloatStr(x)) ;
	}else{
		return Number(changeTwoDecimal(x));
	}
}
//判断账户数是否是整数
function NumIntTwoDecimal(x){
	var ex = /^\d+$/;
	if (ex.test(x)) {
		return x;
	}else{
...
//是否是直辖市  id:页面显示id名称   provinceCode:省（直辖市）编码
function isOnlyCity(id,provinceCode){
...
function timeToChinese(time){
	return time.substring(0,4)+"年"+time.substring(4,6)+"月";
}

function provinceName(prvdCode){
	var provinceName;
    var provList =$.fn.provList();
    // 根据省份ID找到要加载的地图文件
    $.each(provList, function(i, obj) {
       if (obj.provId == prvdCode) {
    	   provinceName = obj.provName;
           return false;
       }
    });
    return provinceName;
...
function getProvinceCode(provinceName){
	...
	// 根据省份ID找到要加载的地图文件
	$.each(provList, function(i, obj) {
		if ( obj.provName == provinceName ) {
			provinceCode = obj.provId;
			return false;
		};
	});
	return provinceCode;
...
Array.prototype.avgValue_= function () {
	var sum=0;
	for (var i = 0; i < this.length; i++)
		sum += Number(this[i]);
	return (sum/this.length).toFixed(2) ;
};
Array.prototype.sumValue_= function () {
	var sum=0;
	for (var i = 0; i < this.length; i++)
		sum +=Number(this[i]);
...
//插码
function insertCodeFun(code){
	var json=$.parseJSON(getCookie("staffInfo")); 
	if(json!=null){
		$.ajax({
			url: $.fn.cmwaurl()+"/base/insertLocalCode",
			async: false,
			data : {"code":code,'staffId':getCookie("staffId"),'staffName':json.staffName},
			dataType: 'json',
			success: function(data) {
			}
		});
	}
	//dcs.addEventCode(code);
}
