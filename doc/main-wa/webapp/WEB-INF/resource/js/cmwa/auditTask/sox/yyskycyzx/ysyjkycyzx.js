...
function initEvent(){
	...
	//图形加载
	$("#hz_ysyjkyzx_tj_chart").click(function() {
		insertCodeFun("MAS_hp_cmwa_hzmx_search_02");

		loadYsyjkyzxChart();
    });
	//折线数据表1
	$("#hz_ysyjkyzx_sjb").click(function() {
		insertCodeFun("MAS_hp_cmwa_hzmx_search_02");

		loadQfycyzx_TabDetailTable("hz_ysyjkyzx_sjb_table","hz_ysyjkyzx_sjb_pageBar");
    });
	
	//折线数导出1
	$("#export_hz_ysyjkyzx_sjb").click(function() {
		insertCodeFun("MAS_hp_cmwa_hzmx_table_download_03");

        var totalNum = $("#hz_ysyjkyzx_sjb_table").getGridParam("records");
        if (!confirm("有{0}条数据，是否要导出？".format(totalNum))) {
            return;
        }
        var postData = {};
        window.location.href = $.fn.cmwaurl() + "/ysyjkycyzx/exportYsyjkyzx_Detail?" + $.param(getHZ_QueryParam()) + "&" + $.param(postData);
        //exportQfycyzx_Detail(exportFileName);
    });
	
	function exportQfycyzx_Detail(exportFileName){
		...
		var url = $.fn.cmwaurl() + "/ysyjkycyzx/exportYsyjkyzx_Detail" +
...
function initDefaultParams(){
    ...
    $.ajax({
        url : $.fn.cmwaurl() + "/ysyjkycyzx/initDefaultParams",
        async : false,
        dataType : 'json',
        data : postData,    
...
function reloadGlobalData(){
	...
	reloadYsyjkyzx_DetailTable();
	
}

// 数据表1
function reloadYsyjkyzx_DetailTable() {
	var postData = getHZ_QueryParam(); 
    var url = $.fn.cmwaurl() + "/ysyjkycyzx/load_ysyjkyzx_TabDetailTable";
    jQuery("#hz_ysyjkyzx_sjb_table").jqGrid('setGridParam', {url: url, postData: postData, page: 1}).trigger("reloadGrid");
}
...
function loadYsyjkyzxChart(){
	var postData = getHZ_QueryParam();
	var id = "hz_ysyjkyzx_chart";
    $.ajax({
        url : $.fn.cmwaurl() + "/ysyjkycyzx/getYsyjkyzxChart",
        dataType : 'json',
        async : false,
        data : postData,    
        success : function(data) {
			...
			for ( var i = 0; i < data_char.length; i++) {
				...
				y_value2.push(data_char[i].sum_dfje);
			}
            drawRankLineChart(x_value,y_value1,y_value2);
            loadAuditConclusion("hz_ysyjkyzx_conclusion",dataCon,postData);
        }
    });
}

function drawRankLineChart(x_value,y_value1,y_value2){
	$('#hz_ysyjkyzx_chart').highcharts({
		chart : {
			zoomType : 'xy'
		},
		title : {
...
function loadAuditConclusion(id,dataCon,postData){
	...
	if(dataCon.length>0){
		...
		if(provinceCode == "10100" || provinceCode == "10400" || provinceCode == "10200" || provinceCode == "10300"){
			text = timeToChinese(postData.hz_startMonth)+"-"+timeToChinese(postData.hz_endMonth)
					+ "，"+$("#provinceName").val()+"财务系统记录预收有价卡款金额"+formatCurrency(sum_dfje,true)
					+ "元，BOSS系统记录预收有价卡款金额" + formatCurrency(sum_yjk_amt,true)+"元，差异金额"+formatCurrency(cyje,true)+"元。";
		}else{	
			text = timeToChinese(postData.hz_startMonth)+"-"+timeToChinese(postData.hz_endMonth)
				+ "，"+$("#provinceName").val()+"省财务系统记录预收有价卡款金额"+formatCurrency(sum_dfje,true)
				+ "元，BOSS系统记录预收有价卡款金额" + formatCurrency(sum_yjk_amt,true)+"元，差异金额"+formatCurrency(cyje,true)+"元。";
		}
	...
	$('#'+id).html(text);
	$("#hz_ysyjkyzx_conclusion").html(text);
	$("#hz_ysyjkyzx_sjb_conclusion").html(text);
}


function loadQfycyzx_TabDetailTable(tableId,pagerId){
	var postData = getHZ_QueryParam();
	
	var tableColNames = [ '审计月','省名称','BOSS预收有价卡款总金额(元)','ERP预收账款-预收有价卡款-贷方金额(元)','差异金额(元)'];
    var tableColModel = [];
    ...
    jQuery("#"+tableId).jqGrid({
    	url : $.fn.cmwaurl() + "/ysyjkycyzx/load_ysyjkyzx_TabDetailTable",
    	datatype: "json",
        postData: postData,
...
$(window).resize(function(){
	$("#hz_ysyjkyzx_sjb_table").setGridWidth($("#ysyjkyzx_DetailTableDiv_class").width()-1);
});
