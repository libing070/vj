...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/cdjllsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/cdjllsjk/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].LOW_GPRS_TOL_G);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/cdjllsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].LOW_GPRS_TOL_G);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/cdjllsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
    ...
    $.each(mapData, function (i, map) {
        ...
        $.each(backdata, function (i, obj) {
        	if(obj.short_name == map.properties.cnname){
        		if(Number(obj.LOW_GPRS_TOL_G) != 0){
        			if(i<=4){
        				map.value = 16;
        			}else if(i>4){
        				map.value = 27;
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计月','省份名称','低价流量使用量(G)','低价流量收入(元)','低价流量平均单价(元/G)','低价流量用户数'];
	
    var tableColModel = [
                    		{name:'AUD_TRM',index:'AUD_TRM',sortable:false},
                    		{name:'SHORT_NAME',index:'SHORT_NAME',sortable:false},
                    		{name:'LOW_GPRS_TOL_G',index:'LOW_GPRS_TOL_G',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'LOW_GPRS_FEE',index:'LOW_GPRS_FEE',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'PER_GPRS_SUBS_G',index:'PER_GPRS_SUBS_G',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'GPRS_SUBS',index:'GPRS_SUBS',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/cdjllsjk/load_table");
}
