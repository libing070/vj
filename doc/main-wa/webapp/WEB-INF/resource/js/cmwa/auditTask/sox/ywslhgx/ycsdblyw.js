   ...
   function initEvent(){
	   ...
	   $("#hz_search_btn").on("click",function(){//汇总页-查询按钮
			...
			if(sumEndDate >= sumBeginDate){
		     ...
		     getTenconClusion();
		     getTenCharts();
		     reLoadSumGridData("#hz_qst_table","/ycsdblyw/load_hz_qst_table");
		     reLoadSumGridData("#hz_yw_table","/ycsdblyw/load_hz_city_table");
		});
	 ...
	 /*  $('#czyCitySelect3').on('click','li',function(){
		   ...
		   var text = $(this).text();
		   $(this).parents('dd').siblings().find(' input').val(text);
		   reLoadSumGridData("#hz_fc_table","/ycsdblyw/hz_fc_table");
		   // getTenconClusion();
		   getFcCharts();
	   });*/
	 ...
	 /*  $("#hz_fc_sjb").on('click',function(){
	   ...
	   $("#hz_qst_export").on("click",function(){//汇总页-虚假开通家庭宽带统计分析--明细tab页--导出
			...
			window.location.href = $.fn.cmwaurl() + "/ycsdblyw/export_hz_qst_table"+"?" + $.param(postData);
		});
	   $("#hz_yw_export").on("click",function(){//汇总页-虚假开通家庭宽带统计分析--明细tab页--导出
		   ...
		   window.location.href = $.fn.cmwaurl() + "/ycsdblyw/export_hz_city_table"+"?" + $.param(postData);
	   });
	   $("#hz_fc_export").on("click",function(){//汇总页-虚假开通家庭宽带统计分析--明细tab页--导出
		   ...
		   window.location.href = $.fn.cmwaurl() + "/ycsdblyw/hz_fc_table_export"+"?" + $.param(postData);
	   });
	   ...
	   $("#mx_search_btn").on("click",function(){//明细页-查询按钮
			...
			*/
			reLoadGridData("#mx_table","/ycsdblyw/load_mx_table");
		});
		...
		$("#export_mx_table").on("click",function(){//明细页-导出按钮
			...
			window.location.href = $.fn.cmwaurl() + "/ycsdblyw/export_mx_table?" + $.param(getDetQueryParam());
		});
   ...
   function getQSTTable(){
	   var postData = getSumQueryParam();
	    var areaCode1 = $('#czyCitySelect2 li.active').attr("date");
		 postData.areaCode1 = areaCode1;
		var tableColNames = [ '审计月','省名称','异常时段办理业务笔数','违规办理操作员数 '];
	    var tableColModel = [
	    ...
	    loadsjbTab(postData, tableColNames, tableColModel, "#hz_qst_table", "#hz_qst_pageBar", "/ycsdblyw/load_hz_qst_table");
   }
   ...
   function getTenTable(){
	   ...
	   var postData = getSumQueryParam();
	   postData.areaCode2 = areaCode2;
	   var tableColNames = [ '审计月','省名称','业务类型名称','异常时段办理业务笔数','违规办理操作员数'];
	   var tableColModel = [
	                        {name:'aud_trm',index:'aud_trm',sortable:false},
	                        {name:'short_name',index:'short_name',sortable:false},
	   ...
	   loadsjbTab(postData, tableColNames, tableColModel, "#hz_yw_table", "#hz_yw_pageBar", "/ycsdblyw/load_hz_city_table");
   }
/*   function getFcTable(){
	   var areaCode3 = $('#czyCitySelect3 li.active').attr("date");
	   var postData = getSumQueryParam();
	   ...
	   loadsjbTab(postData, tableColNames, tableColModel, "#hz_fc_table", "#hz_fc_pageBar", "/ycsdblyw/hz_fc_table");
   }*/
   ...
   function getMxTable(){
	   ...
	   var url= "/ycsdblyw/load_mx_table";
	   var tableColNames = [ '审计月','省名称','操作流水号','业务受理类型名称','用户标识','业务类型名称','业务办理时间',
	                         '办理业务渠道名称','操作员工标识'];
	   var tableColModel = [
	                        {name:'aud_trm',index:'aud_trm',sortable:false},
	                        {name:'short_name',index:'short_name',sortable:false},
   ...
   function getQSTCharts(){
	  // var countNum = $('#jfbsSelect li.active').attr("date");
	   ...
	   var url  = "/ycsdblyw/load_hz_qst_chart";
	   var xValue = [];
	   var numValue = [];
	   var avgNumValue = [];
		   ...
		   success : function(backdata) {
			   if(backdata.length>0){
				   ...
				   if(sum!=0){
					   ...
					   jielun +="异常时段办理业务数量的波动趋势如下。"+ timeToChinese(maxAudTrm) +"，异常时段办理业务数量达到"+formatCurrency(maxNum,false)+"笔，高于平均值"+changeTwoDecimal(avgPer)+"%";
				   }else{
					   jielun+='不存在异常时段办理业务的情况';
				   }
		   }else{
			   jielun += "未上报业务订购相关的数据";
		   }
   ...
   function getTenconClusion(){
	   ...
	   var chnlNameNum = "";
	   $.ajax({
		   url :$.fn.cmwaurl() + "/ycsdblyw/load_hz_city_conclusion",
		   dataType : "json",
		   data : postData,
		   success : function(backdata) {
			   if(backdata.data1!=null&&backdata.data1.busi_num!=undefined){
				   ...
				   if(provinceCode == "10100" || provinceCode == "10400" || provinceCode == "10200" || provinceCode == "10300" ){
					   chnlName="异常时段办理业务"+formatCurrency(backdata.data1.busi_num,false)+"笔，涉及违规办理操作员数"+formatCurrency(backdata.data1.staff_num,false)+"个";
				   }else{
					   chnlName="异常时段办理业务"+formatCurrency(backdata.data1.busi_num,false)+"笔，涉及违规办理操作员数"+formatCurrency(backdata.data1.staff_num,false)+"个。";
					   var top3dist="违规办理业务量排名前三的业务："; 
					   var notop3dist="涉及违规的业务：";
					   switch(len)
					   {
					   case 1:
						   chnlName+=notop3dist+backdata.data2[0].busi_typ_nm+"业务"+formatCurrency(backdata.data2[0].busi_num,false)+"笔";
					     break;
					   case 2:
						   chnlName+=notop3dist+backdata.data2[0].busi_typ_nm+"业务"+formatCurrency(backdata.data2[0].busi_num,false)+"笔，";
						   chnlName+=backdata.data2[1].busi_typ_nm+"业务"+formatCurrency(backdata.data2[1].busi_num,false)+"笔";
						   break;
					   default:
						   chnlName+=top3dist+backdata.data2[0].busi_typ_nm+"业务"+formatCurrency(backdata.data2[0].busi_num,false)+"笔，";
					       chnlName+=backdata.data2[1].busi_typ_nm+"业务"+formatCurrency(backdata.data2[1].busi_num,false)+"笔，";
					       chnlName+=backdata.data2[2].busi_typ_nm+"业务"+formatCurrency(backdata.data2[2].busi_num,false)+"笔";
					   }  
				   }
				  }else{
					  chnlName+="不存在异常时段办理业务的情况";    
				   }
			   }else{
   ...
   function getTenCharts(){
	   var postData = getSumQueryParam();
		$.ajax({
			url :$.fn.cmwaurl() + "/ycsdblyw/load_hz_city_chart",
			dataType : "json",
			data : postData,
			success : function(backdata) {
				var Xvalue = [];
				...
				for(var i = 0;i<backdata.length;i++){
					if(backdata[i]!=null){
						Xvalue.push(backdata[i].busi_typ_nm);
						YOneValue.push(backdata[i].busi_num);
						YTwoValue.push(backdata[i].staff_num);
					}
				}
 ...
 /*  
   function getFcCharts(){
	   ...
	   $.ajax({
		   url :$.fn.cmwaurl() + "/ycsdblyw/hz_fc_chart",
		   dataType : "json",
		   data : postData,
		   success : function(backdata) {
