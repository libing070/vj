...
function initEvent() {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/jtsrycbdsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function initDefaultParams() {
    ...
    var beforeAcctMonth = getBeginDate(endAcctMonth);
...
//根据系统给的结束时间计算起始时间
function getBeginDate(endAcctMonth){
	var endYear = endAcctMonth.substr(0,4);
	var endMouth = endAcctMonth.substr(4,6);
	if(endMouth == 12){
		endMouth = Number(endMouth) - 11 ; 
	}else{
		endMouth = Number(endMouth) + 1;
		endYear = Number(endYear) - 1 ;
	}
	if(endMouth.toString().length == 1){
		endMouth = "0"+endMouth;
	}
	beginDate = endYear + "" + endMouth;
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/jtsrycbdsjk/load_column_chart",
		dataType : "json",
		data : postData,
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/jtsrycbdsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/jtsrycbdsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计起始月','审计结束月','省代码','省名称','集团业务类型','集团业务类型名称','环比波动(%)','方差波动(%)'];
	
    var tableColModel = [
                    		{name:'aud_trm_begin',index:'aud_trm_begin',sortable:false},
                    		{name:'aud_trm_end',index:'aud_trm_end',sortable:false},
                    		{name:'cmcc_prov_prvd_id',index:'cmcc_prov_prvd_id',sortable:false},
                    		{name:'short_name',index:'short_name',sortable:false},
                    		{name:'org_svc_typ',index:'org_svc_typ',sortable:false},
                    		{name:'org_svc_typ_nm',index:'org_svc_typ_nm'},
                    		{name:'rat_wave',index:'rat_wave',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'var_wave',index:'var_wave',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/jtsrycbdsjk/load_table");
}
