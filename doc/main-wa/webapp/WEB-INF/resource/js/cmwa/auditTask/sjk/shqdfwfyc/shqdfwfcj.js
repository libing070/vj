...
function initEvent() {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/shqdcjycsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/shqdcjycsjk/load_column_chart",
		dataType : "json",
		data : postData,
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/shqdcjycsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].subjnum);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/shqdcjycsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计起始月','审计结束月','省名称','销售费用科目编码','销售费用科目名称','环比波动(%)','方差波动(%)'];
    ...
    var tableColModel = [
                    		...
                    		{name:'sell_fee_subj_nm',index:'sell_fee_subj_nm'},
    ...
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/shqdcjycsjk/load_table");
}
