...
function initEvent() {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/shqdsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/shqdsjk/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].SOC_CHNL_NUM);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/shqdsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].SOC_CHNL_NUM);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/shqdsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
...
function loadTable(){
    ...
    var tableColModel = [
                    		...
                    		{name:'VAR_WAVE_RANGE',index:'VAR_WAVE_RANGE',sortable:false},
                    		{name:'SOC_CHNL_NUM',index:'SOC_CHNL_NUM',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
    ...
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/shqdsjk/load_table");
}
