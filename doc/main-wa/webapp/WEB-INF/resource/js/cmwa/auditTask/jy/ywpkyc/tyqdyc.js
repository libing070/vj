	...
	function initEvent(){//step 2：绑定页面元素的响应时间,比如onclick,onchange,hover等
		...
		$("#hz_czy_export").click(function() {
			...
			window.location.href = $.fn.cmwaurl() + "/tyqdyc/export_hz_czy_table?" + $.param(getSumQueryParam());
		});
		...
		$("#export_mx_table").click(function() {
			...
			window.location.href = $.fn.cmwaurl() + "/tyqdyc/export_mx_table?" + $.param(getDetQueryParam());
		});
	...
	function initDefaultParams() {
		...
		$.ajax({
			url : $.fn.cmwaurl() + "/tyqdyc/initDefaultParams",
			async : false,
			dataType : 'json',
			data : postData,
			success : function(data) {
				...
				addSelectEvent("czyCitySingle", function(){
					...
					reLoadSumGridData("#hz_czy_table","/tyqdyc/load_hz_czy_table");
				});
	...
	function reLoadCityDetailGridData(){
		var postData = getDetQueryParam();
		 var url = $.fn.cmwaurl() +"/tyqdyc/load_mx_table";
	...
	function load_hz_qst_chart(){
		...
		$.ajax({
			url :$.fn.cmwaurl() + "/tyqdyc/load_hz_qst_chart",
			dataType : "json",
			data : postData,
			success : function(backdata) {
				...
				if(sum != 0){
					per = (max-avg)/avg*100;
					$("#hz_qst_conclusion").html(timeToChinese(currSumBeginDate)+"-"+timeToChinese(currSumEndDate)+"，"+provinceName(provinceCode)+"同一渠道批量开通业务数量的按月统计趋势如下，其中"+timeToChinese(maxaudtrm)+"，同一渠道批量开通业务数量达到"+formatCurrency(max,false)+"，高于平均值"+changeTwoDecimal(per)+"%。");
				}else{
	...
	function load_hz_czy_chart(){
		...
		var postData = getSumQueryParam();
		$.ajax({
			url :$.fn.cmwaurl() + "/tyqdyc/load_hz_czy_chart",
			dataType : "json",
			data : postData,
			success : function(backdata) {
				var czydata = "[";
				...
				for(var i = 0;i<backdata.length;i++){
					var dangeCzydata = "";
					if(backdata[i]!=null && backdata[i].busi_chnl_id != busi_chnl_id){
						busi_chnl_id =  backdata[i].busi_chnl_id;
						czydata += "{name:'"+backdata[i].busi_chnl_nm+"',y: "+backdata[i].sumNum+", drilldown: '"+backdata[i].busi_chnl_nm+"'},";
						if(backdata[i].busi_chnl_id == busi_chnl_id){
							var flag = 0;
							for(var j = 0;j < backdata.length;j++){
								if(backdata[j].busi_chnl_id == busi_chnl_id){
									++flag;
						...
						dangeCzy +="{ name: '"+backdata[i].busi_chnl_nm+"',id: '"+backdata[i].busi_chnl_nm+"', data:["+ dangeCzydata+"]},";
					}
				}
	...
	function load_hz_czy_conclusion(){
		...
		$.ajax({
			url :$.fn.cmwaurl() + "/tyqdyc/load_hz_czy_conclusion",
			dataType : "json",
			data : postData,
			success : function(backdata) {
				
				if(backdata!=""){
					var conclusion =timeToChinese(currSumBeginDate)+"-"+timeToChinese(currSumEndDate)+"，"+prov_name+"批量开通业务数量排名前三的渠道有";
					for(var i=0;i<backdata.length;i++){
						conclusion += backdata[i].busi_chnl_nm+"、";
					}
	...
	function load_hz_czy_table(){
		var postData = getSumQueryParam();
		var tableColNames = [ '审计区间','省(市)', '办理业务渠道标识 ','办理业务渠道名称', '业务类型编码','业务类型名称','办理业务笔数','天数'];
		var tableColModel = [];
	    ...
	    jQuery("#hz_czy_table").jqGrid({
	        url: $.fn.cmwaurl() + "/tyqdyc/load_hz_czy_table",
	        datatype: "json",
	        postData: postData,
	...
	function load_mx_table(){
		...
		var tableColNames = [ '审计月','省名称','地市名称','办理业务渠道标识','办理业务渠道名称','业务操作流水号','用户标识','业务类型编码','业务类型名称','业务办理时间'];
		var tableColModel = [];
	    ...
	    jQuery("#mx_table").jqGrid({
	        url: $.fn.cmwaurl() + "/tyqdyc/load_mx_table",
	        datatype: "json",
	        postData: postData,
	        colNames: tableColNames,
	        colModel: tableColModel,
