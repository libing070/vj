...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/wgwxzj_qg1601/exportTable"+"?" + $.param(postData);
	});
}
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/wgwxzj_qg1601/load_map_chart",
		dataType : "json",
		data : postData,
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/wgwxzj_qg1601/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/wgwxzj_qg1601/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = [ '审计月','省名称','违规补贴终端数量','违规补贴总成本(元)','总补贴金额(元)'];
    var tableColModel = [
    ...
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/wgwxzj_qg1601/load_table");
}
