...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/yjkzsjzdyc_qg/exportTable"+"?" + $.param(postData);
	});
}
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/yjkzsjzdyc_qg/load_map_chart",
		dataType : "json",
		data : postData,
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/yjkzsjzdyc_qg/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].count_zsyhsl);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/yjkzsjzdyc_qg/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			if(backdata.length>0){
				for(var i=0;i<backdata.length;i++){
					if(backdata[i].aud_trm_end==undefined||backdata[i].count_zsyhsl==undefined){
						xData.push("0");
						yData.push("0");
					}else{
...
function loadTable(){
    ...
    var tableColModel = [
                    		...
                    		{name:'qj_zsje',index:'qj_zsje',sortable:false},
                    		{name:'count_zsyhsl',index:'count_zsyhsl',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/yjkzsjzdyc_qg/load_table");
}
