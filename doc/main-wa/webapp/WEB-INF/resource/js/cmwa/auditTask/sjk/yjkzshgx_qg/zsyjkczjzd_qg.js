...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/zsyjkczjzd_qg/exportTable"+"?" + $.param(postData);
	});
}
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zsyjkczjzd_qg/load_map_chart",
		dataType : "json",
		data : postData,
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zsyjkczjzd_qg/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/zsyjkczjzd_qg/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = [ '审计月','操作省名称','被充值手机号码数量','累计充值有价卡数量','累计充值金额(元)'];
    var tableColModel = [
                    		{name:'aud_trm',index:'aud_trm',sortable:false,width:100},
                    		{name:'short_name',index:'short_name',sortable:false,width:100},
                    		{name:'charge_msisdn_num',index:'zsyjk_cnt',width:200,formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'yjk_num',index:'yjk_num',width:200,formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'yjk_amt',index:'yjk_amt',width:250,formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/zsyjkczjzd_qg/load_table");
}
