...
//			$('#currEntDtBegin').val($('#entDtBegin').val());
			...
			$('#currUserType').val($('#userType li.active').val());
			$('#currCityType').val($('#cityType li.active').val());
			$('#currUserStatus').val($('#userStatus li.active').val());
			$('#currPayType').val($('#payType li.active').val());
...
//		}
		...
		$.ajax({
			url : "/cmwa/realName/initDefaultParams",
			async : false,
			dataType : 'json',
			data : postData,	
			success : function(data) {	
				...
				$("#timedetEnd").html(data['currDetEndDate'].replaceAll("-", ""));
...
//		loadDetailGridData();
	...
	function loadDetailGridData() {
	    ...
	    var tableColNames = [ '审计月', '地市名称', '用户标识','手机号','入网时间','用户状态','入网渠道','付费类型','用户类型', '客户标识'];
	    var tableColModel = [];
	    ...
	    jQuery("#detailGridData").jqGrid({
	        ...
	        loadComplete : function() {
	            // 因本工程样式和jQGrid自身样式有冲突，则对表格特殊处理
	        	 $("#detailGridDataPageBar").css("width", $("#detailGridDataPageBar").width() - 10);
	...
	function loadHighCharts(){
		...
		$.ajax({
	        url : $.fn.cmwaurl() + "/realName/sumRealNamePrvd",
	        dataType : 'json',
	        data : postData,    
	        success : function(data) {
	        	auditList=data.auditList;
				realNamePerList=data.realNamePerList;
				...
				if(listCon.length==0){
					...
					provinceName(provinceCode)+"存量用户实名制率的按月统计趋势如下，"+"其中"+timeToChinese(aud_trm)+"实名比例最低为"+formatCurrency(real_name_per,true)+"%。";
					 $("#conclusion").html(conclusion);
				}
	...
	function loadSumProvince(){
	    ...
	    postData.currSumEndDate=$('#currSumEndDate').val();
		$.ajax({
			url : "/cmwa/realName/summaryDetails",
			dataType: "json",
			data : postData,	
			success : function(backdata) {	
				$('#auditMonth').html(backdata['auditMonth']);
				$('#provName').html(backdata['provName']);
				if(backdata['noRealNameNum']!=''&&backdata['noRealNameNum']!=null&&backdata['noRealNameNum']!="暂无数据"){
					$('#noRealNameNum').html(formatCurrency(backdata['noRealNameNum'],false));
				}else{
					$('#noRealNameNum').html('0');
				}
				
				if(backdata['tolSubsNum']!=''&&backdata['tolSubsNum']!=null&&backdata['tolSubsNum']!="暂无数据"){
				...
				if(backdata['sort']!=''&&backdata['sort']!=null&&backdata['sort']!="暂无数据"){
...
function loadCitySumGridData() {
	            ...
	            $('#' + id).highcharts('Map', {
	                ...
	                colorAxis : {
	                    dataClasses : [
	                        ...
	                        {color : "#ff7979", from : 0.001, to : 90, name : "实名制登记率 < 90%"}
	                    ]
	                },
	                series : [{
	                    animation: {
	...
	function exportDetail(){
		...
		var url = $.fn.cmwaurl() + "/realName/exportDetail" +
	...
	function exportSumCity(){
		...
		var form = $("<form id='filedownload' style='display:none' target='' method='post' />");
		form.attr('action', $.fn.cmwaurl() + "/realName/exportSumCity?currSumBeginDate="+currSumBeginDate+"&currSumEndDate="+currSumEndDate);
		$('body').append(form);
		form.submit();
		form.remove();
	...
	function loadBigHighCharts(){
		...
		$.ajax({
			...
			success : function(backdata) {	
				...
				var realNamePerList = backdata.realNamePerList;
				$('#bigLine').highcharts({
					chart: {
			            zoomType: 'xy',
			            defaultSeriesType: 'line'
