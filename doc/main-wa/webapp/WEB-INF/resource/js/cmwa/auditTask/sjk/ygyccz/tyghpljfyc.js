...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/tyghpljfycsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/tyghpljfycsjk/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].TRADE_CNT);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/tyghpljfycsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].TRADE_CNT);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/tyghpljfycsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
...
function loadTable(){
    ...
    var tableColModel = [
                    		...
                    		{name:'TRADE_CNT',index:'TRADE_CNT',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'TRADE_AMT',index:'TRADE_AMT',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/tyghpljfycsjk/load_table");
}
