...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/hyzdxssjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/hyzdxssjk/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].TRMNL_IMEI_NUM);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/hyzdxssjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			if(backdata.length!=0){
				var year = Number(backdata[0].aud_trm.substr(0,4));
				var mouth = Number(backdata[0].aud_trm.substr(4,6));
				var endyear = Number(backdata[backdata.length-1].aud_trm.substr(0,4));
				var endmouth = Number(backdata[backdata.length-1].aud_trm.substr(4,6));
				var dateLength = (endyear-year)*12+endmouth - mouth;
				xData.push(year+""+backdata[0].aud_trm.substr(4,6));
				...
				for(var i=0;i<xData.length;i++){
					if(!monArray.contains(xData[i])){
						...
						yData.push(backdata[index].TRMNL_IMEI_NUM);
						index += 1;
					}
				}
			}
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/hyzdxssjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
    ...
    $.each(mapData, function (i, map) {
        ...
        $.each(backdata, function (i, obj) {
        	if(obj.short_name == map.properties.cnname){
        		if(Number(obj.TRMNL_IMEI_NUM) != 0){
        			if(i<=4){
        				map.value = 4;
        			}else if(i>4){
        				map.value = 16;
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计月','省名称','重复销售终端IMEI数量'];
	
    var tableColModel = [
                    		{name:'aud_trm',index:'aud_trm',sortable:false},
                    		{name:'short_name',index:'short_name',sortable:false},
                    		{name:'TRMNL_IMEI_NUM',index:'TRMNL_IMEI_NUM',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/hyzdxssjk/load_table");
}
