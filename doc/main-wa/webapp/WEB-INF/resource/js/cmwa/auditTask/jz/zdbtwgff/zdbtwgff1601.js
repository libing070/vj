	...
	function initEvent(){//step 2：绑定页面元素的响应时间,比如onclick,onchange,hover等
		...
		$("#hz_search_btn").on("click",function(){//汇总页-查询按钮
			...
			reLoadSumGridData("#hz_qgpm_sjb_table","/zdbtwgff1601/load_hz_qgpm_tongji_table");
			reLoadSumGridData("#hz_dqfb_sjb_table","/zdbtwgff1601/load_hz_dqfb_sjb_table");
		});
		...
		$("#hz_qgpm_sjb_export").on("click",function(){//汇总页-虚假开通家庭宽带统计分析--明细tab页--导出
			...
			window.location.href = $.fn.cmwaurl() + "/zdbtwgff1601/export_hz_qgpm_sjb?" + $.param(getSumQueryParam());
		});
		$("#hz_dqfb_sjb_export").on("click",function(){//汇总页-虚假开通家庭宽带统计分析--明细tab页--导出
			...
			window.location.href = $.fn.cmwaurl() + "/zdbtwgff1601/export_hz_dqfb_sjb?" + $.param(getSumQueryParam());
		});
		...
		$("#mx_search_btn").on("click",function(){//明细页-查询按钮
			...
			reLoadGridData("#mx_table","/zdbtwgff1601/load_mx_table");
		});
		...
		$("#mx_export_btn").on("click",function(){//明细页-导出按钮
			...
			window.location.href = $.fn.cmwaurl() + "/zdbtwgff1601/export_mx_table?" + $.param(getDetQueryParam());
	...
	function load_hz_qgpm_tongji_chart(){
		...
		$.ajax({
			url :$.fn.cmwaurl() + "/zdbtwgff1601/load_hz_qgpm_tongji_chart",
			dataType : "json",
			data : postData,
	            ...
	            if(backdata==""){
	            	...
	            	for(var i =0; i<backdata.length;i++) {   
	            		...
	            		if(backdata[i].cmccProvPrvdId == provinceCode){
	            			...
	            			paiming = i+1 ;
	            			conclusion = "无线座机、数据卡发放补贴"+formatCurrency(backdata[i].tolAllowCost,true)+"元，涉及终端"
	            				+formatCurrency(backdata[i].trmnlNum,false)+"台，其中包括无线座机"
	            				+formatCurrency(backdata[i].telNum,false)+"台、数据卡"+formatCurrency(backdata[i].cardNum,false)+"张。";
	            		}
            		}
            		for(var i =0; i<backdata.length;i++) {   
	            		...
	            		if(backdata[i].cmccProvPrvdId == provinceCode){
	            			$("#hz_qgpm_tongji_conclusion").html(timeToChinese(currSumBeginDate)+"-"+timeToChinese(currSumEndDate)
	            					+"，"+provinceName(provinceCode)+"向"+conclusion+provinceName(provinceCode)+"在全国排名第"+paiming+"位。");
	            		}
	            	}
...
*/			},
	...
	function hz_dqfb_tongji_chart(){
		...
		var currSumEndDate = $('#currSumEndDate').val();
		$.ajax({
			url :$.fn.cmwaurl() + "/zdbtwgff1601/load_hz_dqfb_tongji_chart",
			dataType : "json",
			data : postData,
			success : function(backdata) {
				var xValue = [];
	            ...
	            if(backdata==""){
	            	if(provinceCode == "10100" || provinceCode == "10400" || provinceCode == "10200" || provinceCode == "10300"){
	        			$("#hz_dqfb_tongji_conclusion").html(timeToChinese(currSumBeginDate)+"- "+timeToChinese(currSumEndDate)+"，"+provinceName(provinceCode)+"无数据。");
	        		}else{
	        			$("#hz_dqfb_tongji_conclusion").html(timeToChinese(currSumBeginDate)+"- "+timeToChinese(currSumEndDate)+"，"+provinceName(provinceCode)+"无数据。");
	        		}
	            }else{
            		...
            		if(provinceCode == "10100" || provinceCode == "10400" || provinceCode == "10200" || provinceCode == "10300"){
            			$("#hz_dqfb_tongji_conclusion").html(timeToChinese(currSumBeginDate)+"- "+timeToChinese(currSumEndDate)+"，"+provinceName(provinceCode)+"向无线座机、数据卡发放补贴"
            					+formatCurrency(allAmt,true)+"元，涉及终端"+formatCurrency(allNum,false)+"台。");
            		}else{
            			$("#hz_dqfb_tongji_conclusion").html(timeToChinese(currSumBeginDate)+"- "+timeToChinese(currSumEndDate)+"，"+provinceName(provinceCode)+"向无线座机、数据卡发放补贴"
            					+formatCurrency(allAmt,true)+"元，涉及终端"+formatCurrency(allNum,false)+"台。补贴金额排名前三的地市："+cityName+"。");
            		}
	            }
...
*/				},
	...
	function load_hz_dqfb_sjb_table(){
		...
		var tableColNames = [ '审计区间','地市名称','向无线座机、数卡发放补贴金额(元)','违规补贴终端数量','Top1渠道名称','Top1渠道向无线座机、数卡发放补贴金额(元)','Top1渠道违规补贴终端数量','Top2渠道名称','Top2渠道向无线座机、数卡发放补贴金额(元)','Top2渠道违规补贴终端数量','Top3渠道名称','Top3渠道向无线座机、数卡发放补贴金额(元)','Top3渠道违规补贴终端数量'];
	    var tableColModel = [];
	    ...
	    loadsjbTab(postData, tableColNames, tableColModel, "#hz_dqfb_sjb_table", "#hz_dqfb_sjb_pageBar", "/zdbtwgff1601/load_hz_dqfb_sjb_table");
	}
	
	//汇总页-统计分析-明细-表格
	function load_hz_qgpm_sjb_table(){
		var postData = this.getSumQueryParam();
		var tableColNames = [ '审计区间','省名称','向无线座机、数卡发放补贴金额(元)','违规补贴终端数量','其中无线座机数量','其中数据卡数量'];
	    var tableColModel = [];
	    ...
	    loadsjbTab(postData, tableColNames, tableColModel, "#hz_qgpm_sjb_table", "#hz_qgpm_sjb_pageBar", "/zdbtwgff1601/load_hz_qgpm_tongji_table");
	}
	
	//明细页-表格
	function load_mx_table(){
		var postData = this.getDetQueryParam();
		var tableColNames = [ '审计月','地市名称','渠道名称','用户标识','终端IMEI','销售时间','实际购机金额(元)','积分兑换值','使用积分兑换金额(元)',
	    ...
	    loadMxTab(postData, tableColNames, tableColModel, "#mx_table", "#mx_pageBar", "/zdbtwgff1601/load_mx_table");
	}
	...
	function initDefaultParams() {
		...
		$.ajax({
			url : $.fn.cmwaurl() + "/zdbtwgff1601/initDefaultParams",
			async : false,
			dataType : 'json',
			data : postData,
			success : function(data) {
				...
				initChnlList("#zongduanType", data['trmnlForm']);
