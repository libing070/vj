...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/yjk200101ss/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/yjk200101ss/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].yjk_cnts);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/yjk200101ss/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].yjk_cnts);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/yjk200101ss/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
        ...
        chart: {
            ...
            events: {
                drillup: function () {
                    ...
                    $('#provname').val('');
                    // 顶部提示按钮隐藏
                    $('#mapTipBtn').closest('.tip_btn_group').show();
                }
            }
        },
        title: {
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = [ '审计月','省名称','批量激活有价卡数量','批量激活有价卡金额(元)','激活有价卡总数量','批量激活有价卡数量占比(%)','激活有价卡总金额(元)','批量激活有价卡金额占比(%)'];
    var tableColModel = [
                    		{name:'aud_trm',index:'aud_trm',sortable:false},
                    		{name:'short_name',index:'short_name',sortable:false},
                    		{name:'yjk_cnt',index:'yjk_cnt',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'amt_sum',index:'amt_sum',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'cnt_tol',index:'cnt_tol',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'per_cnt',index:'per_cnt',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'amt_tol',index:'amt_tol',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'per_amt',index:'per_amt',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/yjk200101ss/load_table");
}
