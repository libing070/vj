...
$(document).ready(function() {
	...
	function LoadDetDetailGridData(){
		    ...
		    var tableColNames = [ '审计月','地市名称','实体渠道标识','实体渠道名称','激励酬金(元)','放号酬金(元)','基础业务服务代理酬金(元)','增值业务代理酬金(元)','终端酬金(元)','房租补贴(元)','发放酬金总额(元)'];
		    var tableColModel = [];
	...
	function loadGsTj(){
		...
		$.ajax({
			...
			success : function(backdata) {
				...
				var bodongzhsStr1 =provinceName(postData.provinceCode)+"在审计期间内激励酬金的比例";
				var amtPer = "";
				var amtPera = "";
				var bodongzhsStr="";
				if(backdata != null && backdata !=""){
					...
					bodongzhsStr =postData.currBeginYear+"年-"+postData.currEndYear+"年期间，"+provinceName(postData.provinceCode)+"的激励酬金占发放酬金金额的比例分别是："+amtPer+"。激励酬金占社会渠道费用预算的比例分别是"+amtPera+"。";
				}else{
					bodongzhsStr ="";
				}
	...
	function loadDsTj(){
		...
		$.ajax({
			...
			success : function(backdata) {
				...
				amtPer = amtPer.substring(0, amtPer.length-1);
				amtPera = amtPera.substring(0, amtPera.length-1);
				var bodongzhsStr =postData.currBeginYear+"年-"+postData.currEndYear+"年期间，"+ds+"的激励酬金占发放酬金金额的比例分别是："+amtPer+"。激励酬金占社会渠道费用预算的比例"+amtPera+"。";
				}else{
				var bodongzhsStr ="";
				}
