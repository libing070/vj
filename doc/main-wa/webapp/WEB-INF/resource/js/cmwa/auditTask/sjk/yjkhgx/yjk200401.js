...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/yjk200401/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/yjk200401/load_column_chart",
		dataType : "json",
		data : postData,
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/yjk200401/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/yjk200401/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计月','充值省份名称','归属省名称','充值月份','异地充值有价卡数量','异地充值的有价卡金额(元)','归属省全部异地充值卡数量','异地充值有价卡数量占比(%)'];
	
    var tableColModel = [

                    		{name:'aud_trm',index:'aud_trm',sortable:false},
                    		{name:'msisdn_prvd_name',index:'msisdn_prvd_name',sortable:false},
                    		{name:'cmcc_prov_prvd_name',index:'cmcc_prov_prvd_name',sortable:false},
                    		{name:'mon',index:'mon',sortable:false},
                    		{name:'tol_cnt',index:'tol_cnt',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		...
                    		{name:'tol_cnt_prvd',index:'tol_cnt_prvd',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
    ...
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/yjk200401/load_table");
}
