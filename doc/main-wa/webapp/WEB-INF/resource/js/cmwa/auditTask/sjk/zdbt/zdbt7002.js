...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
        ...
        alert( $('#exportName').val());
	    var postData = getSumQueryParam();
	    postData.exportName = $('#exportName').val();
		window.location.href = $.fn.cmwaurl() + "/zdbt7002sjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zdbt7002sjk/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].IMEI_CNT2);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/zdbt7002sjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].IMEI_CNT2);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zdbt7002sjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计月','销售省名称','用户数','异常终端IMEI数','营销案终端办理总数','异常占比(%)','违规终端涉及酬金(元)'];
	
    var tableColModel = [
                    		{name:'AUD_TRM',index:'AUD_TRM',sortable:false},
                    		{name:'SHORT_NAME',index:'SHORT_NAME',sortable:false},
                    		{name:'SUBS_CNT2',index:'SUBS_CNT2',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'IMEI_CNT2',index:'IMEI_CNT2',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'IMEI_CNT1',index:'IMEI_CNT1',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		...
                    		{name:'pay_amt',index:'pay_amt',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/zdbt7002sjk/load_table");
}
