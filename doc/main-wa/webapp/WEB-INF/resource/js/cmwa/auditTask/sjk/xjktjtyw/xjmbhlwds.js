...
function initEvent() {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/xjmbsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/xjmbsjk/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].weigui_subs_cnt);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/xjmbsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/xjmbsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计开始月','审计结束月','省名称','疑似虚假魔百和互联网电视用户数','开通魔百和互联网电视用户总数','占比(%)'];
    ...
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/xjmbsjk/load_table");
}
