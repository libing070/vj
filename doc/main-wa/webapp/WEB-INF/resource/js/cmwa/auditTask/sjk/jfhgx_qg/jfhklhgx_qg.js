...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/jfhklhgx_qg/exportTable"+"?" + $.param(postData);
	});
}
...
function initDefaultParams() {
    ...
    endAcctMonth = endAcctMonth.substring(0,4);
    endAcctMonth = (Number(endAcctMonth)-1)+"12";
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/jfhklhgx_qg/load_map_chart",
		dataType : "json",
		data : postData,
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/jfhklhgx_qg/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].per_jfhk);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/jfhklhgx_qg/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			if(backdata.length!=0){
				for(var i=0;i<backdata.length;i++){
					xData.push((backdata[i].aud_trm).substring(0,4)+"年度");
					yData.push(backdata[i].jfhkl);
				}
			}else{
				xData.push(currSumEndDate);
				yData.push(0);
			}
			lineChartData=[];
...
//                text: '违规数量'
            ...
            labels: {
                formatter: function () {
                    return changeTwoDecimal(this.value / 1)+"%" ;
                }
            }
        },
        credits: {
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = [ '审计月','省份名称','用户新增积分','新增积分总价值(元)','用户出账收入(元)','积分回馈率(%)'];
    var tableColModel = [
                    		{name:'aud_trm',index:'aud_trm',sortable:false},
                    		{name:'short_name',index:'short_name',sortable:false},
                    		{name:'curmon_qty_tol',index:'curmon_qty_tol',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'curmon_score_tol',index:'curmon_score_tol',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'mer_tot_fee_tol',index:'mer_tot_fee_tol',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'per_jfhk',index:'per_jfhk',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/jfhklhgx_qg/load_table");
}
