   ...
   function initEvent(){
	   ...
	   $("#hz_tj_export").on("click",function(){//汇总页-虚假开通家庭宽带统计分析--明细tab页--导出
		   ...
		   var totalNum = $("#hz_tj_table").getGridParam("records");
		    ...
		    if(cmccProvId != null && cmccProvId != 0){
				url = "/yjkys_jz/hz_tj_city_table_export";
			}else{
				url = "/yjkys_jz/hz_tj_table_export";
			}
	   ...
	   $("#mx_search_btn").on("click",function(){//明细页-查询按钮
		   insertCodeFun("MAS_hp_cmwa_hzmx_search_02");
			...
			reLoadGridData("#mx_table","/yjkys_jz/mx_table");
		});
	   $("#resetMxId").on("click",function(){//明细页-查询按钮
		...
		$("#export_mx_table").on("click",function(){//明细页-导出按钮
			...
			window.location.href = $.fn.cmwaurl() + "/yjkys_jz/mx_table_export?" + $.param(getDetQueryParam());
		});
   ...
   function getTjTable(){
	    ...
	    var url="";
	    if(cmccProvId != null && cmccProvId != 0){
			url = "/yjkys_jz/hz_tj_city_table";
		}else{
			url = "/yjkys_jz/hz_tj_table";
		}
   ...
   function getTjConclusion(){
	    ...
	    var url = "";
		if(cmccProvId != null && cmccProvId != 0){
			url = "/yjkys_jz/hz_tj_city_table_conclusion";
		}else{
			url = "/yjkys_jz/hz_tj_table_conclusion";
			prvdOrCity = provinceName(provinceCode);
		}
		$.ajax({
			url :$.fn.cmwaurl() + url,
   ...
   function getTjCharts(){
	    ...
	    var prvdOrCity = "";
		if(cmccProvId != null && cmccProvId != 0){
			url = "/yjkys_jz/hz_tj_city_chart";
		}else{
			url = "/yjkys_jz/hz_tj_chart";
			prvdOrCity = provinceName(provinceCode);
		}
		var xValue = [];
		var amtValue = [];
   ...
   function getHzCharts(){
		...
		var url = "";
		if(cmccProvId != null && cmccProvId != 0){
			url = "/yjkys_jz/hz_yc_city_chart";
			prvdOrCity = $('#yjkwczYCSelect li.active').text();
		}else{
			url = "/yjkys_jz/hz_yc_chart";
			prvdOrCity = provinceName(provinceCode);
		}
		var data =[];
		var allamt8=0;
	   ...
	   for(var i = 0; i<data.length;i++){
		   for(var j = 0; j<perRangeType.length;j++){
			   if(perRangeType[j] == data[i].perRange){
				   ...
				   +data[i].prvdOrCity+ "</td><td>" +data[i].perRange+ "</td><td>" 
				   +formatCurrency(changeTwoDecimal(data[i].allamt),true)+ "</td><td>" 
				   +formatCurrency(changeTwoDecimal(data[i].amt),true)+"</td></tr>";
			   }
		   }
		}
	   $(id).html(tr+trno);
   ...
   function drawHighCharts(id,Xvalue,YleftValue,YrightValue){
		$(id).highcharts({
			...
			yAxis : [ {
				labels : {
					formatter: function () {  
                        return formatCurrency(this.value,true) ;
                    }  
				},
				title : {
					text : '营销案对应累计充值金额(元)',
 ...
 //初始化数据
	function initDefaultParams() {
		...
		$.ajax({
			url : $.fn.cmwaurl() + "/yjkys_jz/initDefaultParams",
			async : false,
			dataType : 'json',
			data : postData,
