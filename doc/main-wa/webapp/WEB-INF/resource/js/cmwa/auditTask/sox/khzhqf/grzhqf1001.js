	...
	function initEvent(){//step 2：绑定页面元素的响应时间,比如onclick,onchange,hover等
		...
		$("#hz_search_btn").on("click",function(){//汇总页-查询按钮
			...
			load_qst_chart();
			load_tj_chart();
			reLoadSumGridData("#hz_qst_sjb_table","/grzhqf1001/load_qst_sjb_table");
			reLoadSumGridData("#hz_tj_sjb_table","/grzhqf1001/load_tj_sjb_table");
		});
		...
		$("#hz_qst_sjb_export").on("click",function(){//汇总页-虚假开通家庭宽带统计分析--明细tab页--导出
			...
			window.location.href = $.fn.cmwaurl() + "/grzhqf1001/hz_qst_sjb_export?" + $.param(getSumQueryParam());
		});
		$("#hz_tj_sjb_export").on("click",function(){//汇总页-虚假开通家庭宽带统计分析--明细tab页--导出
			...
			window.location.href = $.fn.cmwaurl() + "/grzhqf1001/hz_tj_sjb_export?" + $.param(getSumQueryParam());
		});
		...
		$("#mx_search_btn").on("click",function(){//明细页-查询按钮
			...
			if($("#currMxActive").val()==0){
				reLoadGridData("#mx_table","/grzhqf1001/load_mx_table_qf");
			}else{
				reLoadGridData("#mx_table","/grzhqf1001/load_mx_table_zb");
			}
		});
		...
		$("#mx_export_btn").on("click",function(){//明细页-导出按钮
	        ...
	        var url = "";
	        if($("#currMxActive").val()==0){
	        	url ="/grzhqf1001/export_mx_table_qf?";
	        }else{
	        	url ="/grzhqf1001/export_mx_table_zb?" ;
	        }
	...
	function load_qst_chart(){
		...
		$.ajax({
			url :$.fn.cmwaurl() + "/grzhqf1001/load_qst_chart",
			dataType : "json",
			data : postData,
...
*/			},
	...
	function load_tj_chart(){
		...
		var currSumEndDate = $('#currSumEndDate').val();
		$.ajax({
			url :$.fn.cmwaurl() + "/grzhqf1001/load_tj_chart",
			dataType : "json",
			data : postData,
			success : function(backdata) {
				var xValue=[];
	...
	function load_qst_sjb_table(){
	    ...
	    loadsjbTab(postData, tableColNames, tableColModel, "#hz_qst_sjb_table", "#hz_qst_sjb_pageBar", "/grzhqf1001/load_qst_sjb_table");
	}
	
	//汇总页-统计分析-明细-表格
	function load_tj_sjb_table(){
		var postData = this.getSumQueryParam();
		var tableColNames = [ '审计月','省名称','帐户标识','用户标识','客户标识','欠费金额(元)','帐本余额(元)','账本余额和欠费金额差值(元)'];
		var tableColModel = [
		                        {name:'aud_trm',index:'aud_trm',sortable:false},
		                        {name:'short_name',index:'short_name',sortable:false},
	    ...
	    loadsjbTab(postData, tableColNames, tableColModel, "#hz_tj_sjb_table", "#hz_tj_sjb_pageBar", "/grzhqf1001/load_tj_sjb_table");
	}
	...
	function load_mx_table_qf(){
		...
		var tableColNames = [ '审计月','省名称','帐户标识','用户标识','客户标识','欠费帐期','欠费科目','欠费金额(元)'];
		var tableColModel = [
		                        ...
		                        {name:'subs_id',index:'subs_id',sortable:false,width:200},
		                        {name:'cust_id',index:'cust_id',sortable:false,width:200},
		                        {name:'acct_prd_ytm',index:'acct_prd_ytm',sortable:false,width:100},
		                        {name:'compt_accts_subj_nm',index:'compt_accts_subj_nm',sortable:false},
		                        {name:'dbt_amt',index:'dbt_amt',  formatter: "integer", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false}
		                        ];
	    loadMxTab(postData, tableColNames, tableColModel, "#mx_table", "#mx_pageBar", "/grzhqf1001/load_mx_table_qf");
	}
	//明细页-表格
	function load_mx_table_zb(){
		$('#shuju_table').html('<table id="mx_table"></table><div id="mx_pageBar"></div>');
		var postData = this.getDetQueryParam();
		var tableColNames = [ '审计月','省名称','帐户标识','用户标识','客户标识','账本科目','账本余额(元)'];
		var tableColModel = [
		...
		loadMxTab(postData, tableColNames, tableColModel, "#mx_table", "#mx_pageBar", "/grzhqf1001/load_mx_table_zb");
	}
	...
	function initDefaultParams() {
		...
		$.ajax({
			url : $.fn.cmwaurl() + "/grzhqf1001/initDefaultParams",
			async : false,
			dataType : 'json',
			data : postData,
