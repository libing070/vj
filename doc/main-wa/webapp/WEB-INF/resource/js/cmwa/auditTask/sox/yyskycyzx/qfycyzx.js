...
function initEvent(){
	...
	//图形加载
	$("#hz_qfycyzx_tj_chart").click(function() {
		insertCodeFun("MAS_hp_cmwa_hzmx_search_02");

		loadQfycyzxChart();
    });
	//折线数据表1
	$("#hz_qfycyzx_sjb").click(function() {
		insertCodeFun("MAS_hp_cmwa_hzmx_search_02");

		loadQfycyzx_TabDetailTable("hz_qfycyzx_sjb_table","hz_qfycyzx_sjb_pageBar");
    });
	
	//折线数导出1
	$("#export_hz_qfycyzx_sjb").click(function() {
		insertCodeFun("MAS_hp_cmwa_hzmx_table_download_03");

        var totalNum = $("#hz_qfycyzx_sjb_table").getGridParam("records");
        if (!confirm("有{0}条数据，是否要导出？".format(totalNum))) {
            return;
        }
        var postData = {};
        window.location.href = $.fn.cmwaurl() + "/qfycyzx/exportQfycyzx_Detail?" + $.param(getHZ_QueryParam()) + "&" + $.param(postData);
	...
	function exportQfycyzx_Detail(exportFileName){
		...
		var url = $.fn.cmwaurl() + "/qfycyzx/exportQfycyzx_Detail" +
...
function initDefaultParams(){
    ...
    $.ajax({
        url : $.fn.cmwaurl() + "/qfycyzx/initDefaultParams",
        async : false,
        dataType : 'json',
        data : postData,    
...
function reloadCqqfwtjzhs_sf_DetailTable() {
	var postData = getHZ_QueryParam(); 
    var url = $.fn.cmwaurl() + "/qfycyzx/load_qfycyzx_TabDetailTable";
    jQuery("#hz_qfycyzx_sjb_table").jqGrid('setGridParam', {url: url, postData: postData, page: 1}).trigger("reloadGrid");
}
...
function loadQfycyzxChart(){
	var postData = getHZ_QueryParam();
	var id = "hz_qfycyzx_chart";
    $.ajax({
        url : $.fn.cmwaurl() + "/qfycyzx/getCwxtqftjChart",
        dataType : 'json',
        async : false,
        data : postData,    
        success : function(data) {
            ...
            loadAuditConclusion("hz_qfycyzx_conclusion",dataCon,postData);
        }
    });
}

function drawRankLineChart(x_value,y_value1,y_value2){
	$('#hz_qfycyzx_chart').highcharts({
		chart : {
			zoomType : 'xy'
		},
		title : {
...
function loadAuditConclusion(id,dataCon,postData){
	...
	if(dataCon.length>0){
		...
		if(provinceCode == "10100" || provinceCode == "10400" || provinceCode == "10200" || provinceCode == "10300"){
			text = timeToChinese(postData.hz_startMonth)+"-"+timeToChinese(postData.hz_endMonth)
					+ "，"+$("#provinceName").val()+ "财务系统记录欠费金额" + formatCurrency(sum_qmye,true)
					+ "元，BOSS系统记录欠费金额"+formatCurrency(sum_yjk_amt,true)+"元，差异金额"+formatCurrency(cyje,true)+"元。";
		}else{	
			text = timeToChinese(postData.hz_startMonth)+"-"+timeToChinese(postData.hz_endMonth)
				+ "，"+$("#provinceName").val()+ "省财务系统记录欠费金额" + formatCurrency(sum_qmye,true)
				+ "元，BOSS系统记录欠费金额"+formatCurrency(sum_yjk_amt,true)+"元，差异金额"+formatCurrency(cyje,true)+"元。";
		}
	...
	$('#'+id).html(text);
	$("#hz_qfycyzx_conclusion").html(text);
	$("#hz_qfycyzx_sjb_conclusion").html(text);
...
function loadQfycyzx_TabDetailTable(tableId,pagerId){
	...
	var tableColNames = [ '审计月','省名称','BOSS欠费总金额(元)','ERP应收账款-用户欠费-期末余额(元)','差异金额(元)'];
    var tableColModel = [];
    ...
    jQuery("#"+tableId).jqGrid({
    	url : $.fn.cmwaurl() + "/qfycyzx/load_qfycyzx_TabDetailTable",
    	 datatype: "json",
         postData: postData,
...
$(window).resize(function(){
	$("#hz_qfycyzx_sjb_table").setGridWidth($("#qfycyzx_DetailTableDiv_class").width()-1);
});
