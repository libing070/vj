...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/zsyjkyscz_qg/exportTable"+"?" + $.param(postData);
	});
}
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zsyjkyscz_qg/load_map_chart",
		dataType : "json",
		data : postData,
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zsyjkyscz_qg/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].offer_zsyjk_ys_amt);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/zsyjkyscz_qg/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].offer_zsyjk_ys_amt);
					index += 1;
				}
			}
			lineChartData=[];
    ...
    $.each(mapData, function (i, map) {
        ...
        $.each(backdata, function (i, obj) {
        	if(obj.short_name == map.properties.cnname){
        		if(Number(obj.offer_zsyjk_ys_amt) != 0 ){
        			if(i<=4){
        				map.value = 16;
        			}else if(i>4){
        				map.value = 27;
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = [ '审计月','归属省名称','充值省名称','营销案名称','赠送有价卡累计异省充值数量','赠送有价卡累计异省充值金额(元)'];
    var tableColModel = [
                    		{name:'aud_trm',index:'aud_trm',sortable:false,width:100},
                    		{name:'short_name',index:'short_name',sortable:false,width:110},
                    		{name:'opr_prvd_nm',index:'opr_prvd_nm',sortable:false,width:110},
                    		{name:'offer_nm',index:'offer_nm',sortable:false},
                    		{name:'offer_zsyjk_ys_num',index:'offer_zsyjk_ys_num',width:200,formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'offer_zsyjk_ys_amt',index:'offer_zsyjk_ys_amt',width:200,formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/zsyjkyscz_qg/load_table");
}
