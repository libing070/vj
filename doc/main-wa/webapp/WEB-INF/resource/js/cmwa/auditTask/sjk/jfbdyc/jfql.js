...
function initEvent() {
    ...
    $("#table_export").on("click",function(){//tab页--导出
	    ...
	    var str=postData.currSumBeginDate.substring(0,4);
		postData.currSumBeginDate=str;
		window.location.href = $.fn.cmwaurl() + "/jfqlsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	...
	postData.currSumBeginDate=str;
	$.ajax({
		url :$.fn.cmwaurl() + "/jfqlsjk/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].zero_pres_per);
			}
...
function drowLineChart(){
	...
	var str=currSumBeginDate.substring(0,4);
	postData.currSumBeginDate=str;
	$.ajax({
		url :$.fn.cmwaurl() + "/jfqlsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
		...
		/*	var monArray = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].zero_pres_per);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	...
	postData.currSumBeginDate=str;
	$.ajax({
		url :$.fn.cmwaurl() + "/jfqlsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
...
function loadTable(){
    ...
    var str=postData.currSumBeginDate.substring(0,4);
	postData.currSumBeginDate=str;
	var tableColNames = ['审计起始月','审计结束月','省名称','赠送积分值','清零积分值','清零/赠送比例(%)'];
    ...
    var tableColModel = [
                    		...
                    		{name:'pres_value',index:'pres_value',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'zero_value',index:'zero_value',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'zero_pres_per',index:'zero_pres_per',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/jfqlsjk/load_table");
}
