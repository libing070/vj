...
function getSumRepeat(){
	...
	$.ajax({
		url : $.fn.cmwaurl() + "/hyzdbthgx/1702/getSumRepeat",
		dataType : 'json',
		data : postData,
		success : function(data) {
...
function getSumPieCharts(){
	var postData = getSumQueryParam();
	$.ajax({
		url : $.fn.cmwaurl() + "/hyzdbthgx/1702/getSumPieCharts",
		dataType : 'json',
		data : postData,
		success : function(data) {
...
function loadPieCon(postData,data){
	var text = timeToChinese(postData.hz_startMonth) + "-" + timeToChinese(postData.hz_endMonth)
		+"，"+$("#provinceName").val()+"办理终端类合约计划"+formatCurrency(data.SUM_TOL_IMEI,false)
		+"笔，其中重复办理"+formatCurrency(data.SUM_IMEI_NUM,false)+"笔。重复办理次数对应的用户数分布如下。";
	$('#pieCon').html(text);
...
function loadSumRepeatCon(postData,data){
	...
	if(provinceCode == "10100" || provinceCode == "10400" || provinceCode == "10200" || provinceCode == "10300" ){
		text = timeToChinese(postData.hz_startMonth) + "-" + timeToChinese(postData.hz_endMonth)+"，"+$("#provinceName").val()+formatCurrency(data.sumMap.SUM_SUB_NUM,false)+"个用户重复办理终端类合约计划，涉及终端"+formatCurrency(data.sumMap.SUM_IMEI_NUM,false)+"台.";
	}else{
		text = timeToChinese(postData.hz_startMonth) + "-" + timeToChinese(postData.hz_endMonth)+"，"+$("#provinceName").val()+formatCurrency(data.sumMap.SUM_SUB_NUM,false)+"个用户重复办理终端类合约计划，涉及终端"+formatCurrency(data.sumMap.SUM_IMEI_NUM,false)+"台。重复办理合约计划终端数量排名前三的地市："+top3city+"涉及终端"+formatCurrency(top3sum,false)+"台";
	}
...
function loadSumTable(tableId,pagerId) {
    ...
    jQuery("#"+tableId).jqGrid({
    	url : $.fn.cmwaurl() + "/hyzdbthgx/1702/getSumRepeatTable",
        datatype: "json",
        postData: postData,
...
function loadDetailTable(tableId,pagerId){
    ...
    jQuery("#"+tableId).jqGrid({
    	url : $.fn.cmwaurl() + "/hyzdbthgx/1702/getDetailListPager",
        datatype: "json",
        postData: postData,
        colNames: tableColNames,
        colModel: tableColModel,
...
function reLoadCityDetailGridData(tableId){
	  var postData = getDetailQueryParam();
	  var url = $.fn.cmwaurl() + "/hyzdbthgx/1702/getDetailListPager";
...
function exportTab(functionName){
	...
	var url = $.fn.cmwaurl() + "/hyzdbthgx/1702/" +functionName+
