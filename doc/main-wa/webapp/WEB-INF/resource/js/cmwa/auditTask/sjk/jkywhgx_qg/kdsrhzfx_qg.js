...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/kdsrhzfx_qg/exportTable"+"?" + $.param(postData);
	});
}
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/kdsrhzfx_qg/load_map_chart",
		dataType : "json",
		data : postData,
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/kdsrhzfx_qg/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].sun_bill_amt);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/kdsrhzfx_qg/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].sun_bill_amt);
					index += 1;
				}
			}
			lineChartData=[];
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = [ '审计月','省代码','省名称','业务类型','用户数','账单收入(元)'];
    var tableColModel = [
                    		{name:'aud_trm',index:'aud_trm',sortable:false},
                    		{name:'cmcc_prov_prvd_id',index:'short_name',sortable:false},
                    		{name:'short_name',index:'short_name',sortable:false},
                    		{name:'busn_typ',index:'busn_typ',sortable:false},
                    		{name:'subs_num',index:'tol_num',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'sun_bill_amt',index:'sun_bill_amt',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/kdsrhzfx_qg/load_table");
}
