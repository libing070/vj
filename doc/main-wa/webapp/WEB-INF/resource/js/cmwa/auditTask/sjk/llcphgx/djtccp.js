...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/djtccpsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/djtccpsjk/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].INFRAC_PACK_NUM);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/djtccpsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].INFRAC_PACK_NUM);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/djtccpsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
    ...
    $.each(mapData, function (i, map) {
        ...
        $.each(backdata, function (i, obj) {
        	if(obj.short_name == map.properties.cnname){
        		if(Number(obj.INFRAC_PACK_NUM) != 0){
        			if(i<=4){
        				map.value = 16;
        			}else if(i>4){
        				map.value = 27;
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计月','省名称','违规资费套餐数量','当月新上线资费套餐数量','当月违规订购用户数'];
    var tableColModel = [
                    		{name:'AUD_TRM',index:'AUD_TRM',sortable:false},
                    		{name:'SHORT_NAME',index:'SHORT_NAME',sortable:false},
                    		{name:'INFRAC_PACK_NUM',index:'INFRAC_PACK_NUM',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'NEW_PACK_NUM',index:'NEW_PACK_NUM',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'PACK_USER_NUM',index:'PACK_USER_NUM',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/djtccpsjk/load_table");
}
