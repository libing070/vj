...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/wgdfdzzdff_qg1602/exportTable"+"?" + $.param(postData);
	});
}
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/wgdfdzzdff_qg1602/load_map_chart",
		dataType : "json",
		data : postData,
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/wgdfdzzdff_qg1602/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/wgdfdzzdff_qg1602/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].tol_amt);
					index += 1;
				}
			}
			lineChartData=[];
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = [ '审计月','省名称','违规补贴非4G定制终端数量','终端补贴成本(元)','总补贴金额(元)'];
    var tableColModel = [
                    		{name:'aud_trm',index:'aud_trm',sortable:false,width:100},
                    		{name:'short_name',index:'short_name',sortable:false,width:100},
                    		{name:'trmnl_num',index:'trmnl_num',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'trmnl_allow_cost',index:'trmnl_allow_cost',formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'tol_amt',index:'tol_amt',formatter: "currency", width:200,formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/wgdfdzzdff_qg1602/load_table");
}
