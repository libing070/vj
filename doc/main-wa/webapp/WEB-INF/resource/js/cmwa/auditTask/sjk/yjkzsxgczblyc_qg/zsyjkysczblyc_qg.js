...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/zsyjkysczblyc_qg/exportTable"+"?" + $.param(postData);
	});
}
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zsyjkysczblyc_qg/load_map_chart",
		dataType : "json",
		data : postData,
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zsyjkysczblyc_qg/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].count_offer_cd);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/zsyjkysczblyc_qg/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<backdata.length;i++){
				monArray.push((backdata[i].aud_trm).substring(0,4)+"年度");
			}
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					yData.push(0);
				}else{
					yData.push(backdata[index].count_offer_cd);
					index += 1;
				}
			}
			lineChartData=[];
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = [ '审计起始月','审计结束月','省名称','异省充值金额比例≥60%的营销案数量'];
    var tableColModel = [
                    		{name:'currSumBeginDate',index:'currSumBeginDate',sortable:false},
                    		{name:'currSumEndDate',index:'currSumEndDate',sortable:false},
                    		{name:'short_name',index:'short_name',sortable:false},
                    		{name:'count_offer_cd',index:'count_offer_cd',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/zsyjkysczblyc_qg/load_table");
}
