...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/zdkcsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zdkcsjk/load_column_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			...
			for(var i=0;i<backdata.length;i++){
				...
				yData.push(backdata[i].long_aging_imei_num);
			}
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/zdkcsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].long_aging_imei_num);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/zdkcsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
    ...
    $.each(mapData, function (i, map) {
        ...
        $.each(backdata, function (i, obj) {
        	if(obj.short_name == map.properties.cnname){
        		if(Number(obj.long_aging_imei_num) != 0){
        			if(i<=4){
        				map.value = 16;
        			}else if(i>4){
        				map.value = 27;
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计月','省名称','入库时间','长账龄库存终端数量'];
    var tableColModel = [

                    		{name:'aud_trm',index:'aud_trm',sortable:false},
                    		{name:'short_name',index:'short_name',sortable:false},
                    		{name:'dat_rcd_dt',index:'dat_rcd_dt',sortable:false},
                    		{name:'long_aging_imei_num',index:'long_aging_imei_num',formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/zdkcsjk/load_table");
}
