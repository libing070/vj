	...
	function initEvent(){//step 2：绑定页面元素的响应时间,比如onclick,onchange,hover等
		...
		$('#jfbsSelect').on('click','li',function(){
			...
			$('#jfbsVal').val($('#jfbsText').val());
			load_hz_qst_chart();
			reLoadSumGridData("#hz_qst_table","/tyghjfyc/load_hz_qst_table");
		});
		//导出波动趋势数据表
		$("#hz_qst_export").click(function() {
			...
			window.location.href = $.fn.cmwaurl() + "/tyghjfyc/export_hz_qst_table?" + $.param(getSumQueryParam());
		});
		//导出波动趋势数据表
		$("#hz_czy_export").click(function() {
			...
			window.location.href = $.fn.cmwaurl() + "/tyghjfyc/export_hz_czy_table?" + $.param(getSumQueryParam());
		});
		...
		$("#export_mx_table").click(function() {
			...
			window.location.href = $.fn.cmwaurl() + "/tyghjfyc/export_mx_table?" + $.param(getDetQueryParam());
		});
		...
		$("#queryButton").click(function() {
			...
			var busiType = $("#busiType li.active").attr("code");
	...
	function initDefaultParams() {
		...
		$.ajax({
			url : $.fn.cmwaurl() + "/tyghjfyc/initDefaultParams",
			async : false,
			dataType : 'json',
			data : postData,
			success : function(data) {
				...
				addSelectEvent("czyCitySingle", function(){
					...
					reLoadSumGridData("#hz_czy_table","/tyghjfyc/load_hz_czy_table");
				});
	...
	function initUl(idStr,data){
		...
		if(len!=0){
		    for ( var i = 0; i < len; i++) {
		    	liStr +="<li code='"+data[i].value+"'>"+data[i].text+"</li>";
		    }
		}
		if(len==0){  
			liStr +="<li>暂无数据</li>";
	...
	function initJFBSList(id){
		var liStr = "";
		liStr += "<li code=''>缴费笔数阀值</li>";
		liStr += "<li code='50'>50</li>";
		liStr += "<li code='55'>55</li>";
		liStr += "<li code='60'>60</li>";
		liStr += "<li code='65'>65</li>";
		liStr += "<li code='70'>70</li>";
		liStr += "<li code='75'>75</li>";
		liStr += "<li code='80'>80</li>";
		liStr += "<li code='85'>85</li>";
		liStr += "<li code='90'>90</li>";
		liStr += "<li code='95'>95</li>";
		liStr += "<li code='100'>100</li>";
		$(id).html(liStr);
	}
	/**
	 * 为指定ID添加下拉框监听事件
	...
	function reLoadCityDetailGridData(){
		var postData = getDetQueryParam();
		 var url = $.fn.cmwaurl() +"/tyghjfyc/load_mx_table";
	...
	function getDetQueryParam(){
		...
		postData.currBusiType = $('#currBusiType').val();
	...
	function load_hz_qst_chart(){
		...
		$.ajax({
			url :$.fn.cmwaurl() + "/tyghjfyc/load_hz_qst_chart",
			dataType : "json",
			data : postData,
			success : function(backdata) {
				...
				for(var i = 0;i<backdata.length;i++){
					if(backdata[i]!=null){
						...
						YOneValue.push(backdata[i].tradeCnt);
					}
				}
				var avg = YOneValue.avgValue();
				var max = YOneValue.maxValue();
				var per = (max - avg)/avg*100;
				for(var i = 0;i<backdata.length;i++){
					if(backdata[i]!=null){
						YTwoValue.push(Number(avg));
						if(backdata[i].tradeCnt == max){
				...
				if(backdata.length != 0){
					$("#hz_qst_conclusion").html(timeToChinese(currSumBeginDate)+"-"+timeToChinese(currSumEndDate)+"，"+provinceName(provinceCode)+"同一工号批量缴费业务数量的按月统计趋势如下，其中"+timeToChinese(maxAudtrm)+"，同一工号批量缴费业务数量达到"+formatCurrency(max,false)+"，高于平均值"+per.toFixed(2)+"%。");
					$("#hz_qst_table_conclusion").html(timeToChinese(currSumBeginDate)+"-"+timeToChinese(currSumEndDate)+"，"+provinceName(provinceCode)+timeToChinese(maxAudtrm)+"同一工号批量缴费业务数量达到"+formatCurrency(max,false)+"，高于平均值"+per.toFixed(2)+"%。");
				}else{
	...
	function load_hz_qst_table(){
	    ...
	    jQuery("#hz_qst_table").jqGrid({
	        url: $.fn.cmwaurl() + "/tyghjfyc/load_hz_qst_table",
	        datatype: "json",
	        postData: postData,
	...
	function load_hz_czy_chart(){
		var postData = getSumQueryParam();
		$.ajax({
			url :$.fn.cmwaurl() + "/tyghjfyc/load_hz_czy_chart",
			dataType : "json",
			data : postData,
			success : function(backdata) {
				var czydata = "[";
				...
				for(var i = 0;i<backdata.length;i++){
					var dangeCzydata = "";
					if(backdata[i]!=null && backdata[i].EMP_ID != staffid){
						staffid =  backdata[i].EMP_ID;
						czydata += "{name:'"+backdata[i].EMP_NM+"',y: "+backdata[i].sumNum+", drilldown: '"+backdata[i].EMP_ID+"'},";
							for(var j = 0;j<backdata.length;j++){
								if(backdata[j].EMP_ID == staffid){
									dangeCzydata += "['"+backdata[j].busi_typ_nm+"',"+backdata[j].TRADE_CNT+"],";
								}
							}
							dangeCzydata = dangeCzydata.substring(0, dangeCzydata.length-1);
						dangeCzy +="{ name: '"+backdata[i].EMP_NM+"',id: '"+backdata[i].EMP_ID+"', data:["+ dangeCzydata+"]},";
					}
				}
	...
	function load_hz_czy_conclusion(){
		...
		$.ajax({
			url :$.fn.cmwaurl() + "/tyghjfyc/load_hz_czy_conclusion",
			dataType : "json",
			data : postData,
			success : function(backdata) {
				if(backdata!=""){
					var conclusion =timeToChinese(currSumBeginDate)+"-"+timeToChinese(currSumEndDate)+"，"+prov_name+"批量缴费业务数量排名前三的员工有";
					for(var i=0;i<backdata.length;i++){
					...
					conclusion += "，分别办理";
					for(var i=0;i<backdata.length;i++){
						conclusion += formatCurrency(backdata[i].busitypnum,false)+"种类型的缴费业务"+formatCurrency(backdata[i].sumNum,false)+"笔、";
					}
					conclusion = conclusion.substring(0, conclusion.length-1);
					conclusion += "。";
	...
	function load_hz_czy_table(){
		var postData = getSumQueryParam();
		var tableColNames = [ '审计区间','省(市)', '员工标识 ','姓名', '业务类型编码','业务类型名称','缴费笔数','天数','交易金额(元)'];
		var tableColModel = [];
	    ...
	    jQuery("#hz_czy_table").jqGrid({
	        url: $.fn.cmwaurl() + "/tyghjfyc/load_hz_czy_table",
	        datatype: "json",
	        postData: postData,
	        colNames: tableColNames,
	        colModel: tableColModel,
	...
	function load_mx_table(){
		...
		var tableColNames = [ '审计月','省名称','地市名称','员工标识','姓名','交易流水号','用户标识','业务类型编码','业务类型名称','交易金额(元)','业务办理时间'];
		var tableColModel = [];
	    ...
	    jQuery("#mx_table").jqGrid({
	        url: $.fn.cmwaurl() + "/tyghjfyc/load_mx_table",
	        datatype: "json",
	        postData: postData,
	        colNames: tableColNames,
	        colModel: tableColModel,
