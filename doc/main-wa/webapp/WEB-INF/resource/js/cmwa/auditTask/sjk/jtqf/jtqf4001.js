...
function initEvent() {
   ...
   /* $("#allDataBtn").on("click", function () {
    ...
    $("#table_export").on("click",function(){//tab页--导出
		...
		window.location.href = $.fn.cmwaurl() + "/jtqfsjk/exportTable"+"?" + $.param(postData);
	});
}
...
function drowColumnChart(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/jtqfsjk/load_column_chart",
		dataType : "json",
		data : postData,
...
function drowLineChart(){
	...
	$.ajax({
		url :$.fn.cmwaurl() + "/jtqfsjk/load_line_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
			var xData = [];
			...
			for(var i=0;i<xData.length;i++){
				if(!monArray.contains(xData[i])){
					...
					yData.push(backdata[index].dbt_amt);
					index += 1;
				}
			}
			lineChartData=[];
...
function drowChinaMap(){
	var postData = getSumQueryParam();
	$.ajax({
		url :$.fn.cmwaurl() + "/jtqfsjk/load_map_chart",
		dataType : "json",
		data : postData,
		success : function(backdata) {
...
function loadTable(){
    var postData = getSumQueryParam();
	var tableColNames = ['审计月','省名称','欠费集团账户数','账户数环比(%)','欠费总金额(元)','金额环比(%)'];
	
    var tableColModel = [

                    		{name:'aud_trm',index:'aud_trm',sortable:false,width:100},
                    		{name:'short_name',index:'short_name',sortable:false,width:100},
                    		{name:'acct_num',index:'acct_num',width:200,formatter: "currency",formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:0},sortable:false},
                    		{name:'acct_num_mom',index:'acct_num_mom',width:100,formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'dbt_amt',index:'dbt_amt',width:200,formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                    		{name:'dbt_amt_mom',index:'dbt_amt_mom',width:00,formatter: "currency", formatoptions: {thousandsSeparator:",",decimalSeparator:".",decimalPlaces:2},sortable:false},
                         ];
    
    loadsjbTab(postData, tableColNames, tableColModel, "#tableId", "#table_pageBar", "/jtqfsjk/load_table");
}
