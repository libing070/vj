   ...
   function initEvent(){
	   ...
	   $("#hz_search_btn").on("click",function(){//汇总页-查询按钮
			...
			if(sumEndDate >= sumBeginDate){
		     ...
		     getTenCharts();
		     reLoadSumGridData("#hz_yw_table","/xjmbhlwds/load_hz_czy_table");
/*		     reLoadSumGridData("#hz_yw_table","/xjmbhlwds/load_hz_city_table");
*/		});
	   ...
	   $("#hz_yw_export").on("click",function(){//汇总页-虚假开通家庭宽带统计分析--明细tab页--导出
		   ...
		   window.location.href = $.fn.cmwaurl() + "/xjmbhlwds/export_hz_czy_table"+"?" + $.param(postData);
	   });
	   ...
	   $("#mx_search_btn").on("click",function(){//明细页-查询按钮
			...
			*/
			reLoadGridData("#mx_table","/xjmbhlwds/load_mx_table");
		});
		...
		$("#export_mx_table").on("click",function(){//明细页-导出按钮
			...
			window.location.href = $.fn.cmwaurl() + "/xjmbhlwds/export_mx_table?" + $.param(getDetQueryParam());
		});
   ...
   function getTenTable(){
	   ...
	   var postData = getSumQueryParam();
	   postData.areaCode2 = areaCode2;
	   var tableColNames = ['审计开始月','审计结束月','省名称','地市名称','疑似虚假魔百和互联网电视用户数','开通魔百和互联网电视用户总数','占比(%)'];
	   var tableColModel = [
	   ...
	   loadsjbTab(postData, tableColNames, tableColModel, "#hz_yw_table", "#hz_yw_pageBar", "/xjmbhlwds/load_hz_czy_table");
   }
   ...
   function getMxTable(){
	   ...
	   var url= "/xjmbhlwds/load_mx_table";
   ...
   function getTenconClusion(backdata2){
	   ...
	   var chnlNameNum = "";
	   $.ajax({
		   url :$.fn.cmwaurl() + "/xjmbhlwds/load_hz_conclusion",
		   dataType : "json",
		   data : postData,
		   success : function(backdata) {
			   if(backdata[0]!=null){
				   ...
				   if(provinceCode == "10100" || provinceCode == "10400" || provinceCode == "10200" || provinceCode == "10300" ){
					   chnlName="疑似虚假开通魔百和互联网电视"+formatCurrency(backdata[0].weiguiCnt,false)+"笔，占比"+formatCurrency(backdata[0].zper,true)+"%";
				   }else{
					   chnlName="疑似虚假开通魔百和互联网电视"+formatCurrency(backdata[0].weiguiCnt,false)+"笔，占比"+formatCurrency(backdata[0].zper,true)+"%。";
					    var wg_busi_num=0;
					    var wg_cust_num=0;
					    var len=backdata2.length;
					   var top3dist="疑似虚假开通魔百和互联网电视用户数排名前三的地市："; 
					   var notop3dist="涉及虚假开通的地市：";
					   switch(len)
					   {
					   case 1:
					   ...
					   default:
					       ...
					       chnlName+=backdata2[1].cmcc_prvd_nm_short+"、";
					       chnlName+=backdata2[2].cmcc_prvd_nm_short+"，涉及"+formatCurrency((backdata2[0].weiguiCnt+backdata2[1].weiguiCnt+backdata2[2].weiguiCnt),false)+"个魔百和用户";
					   }  
				   }
				  }else{
					  chnlName+="未发现虚假开通魔百和互联网电视业务的情况";  
				  }
			   }else{
				   chnlName = "未上报魔百和互联网电视开通数据";
			   }
   ...
   function getTenCharts(){
	   var postData = getSumQueryParam();
		$.ajax({
			url :$.fn.cmwaurl() + "/xjmbhlwds/load_hz_qst_chart",
			dataType : "json",
			data : postData,
			success : function(backdata) {
