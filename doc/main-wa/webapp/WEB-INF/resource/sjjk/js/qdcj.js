...
*/	//1 个性化页面的特殊风格
	...
	function getCurrDateBefore2Month(){var result = [];
	 var d = new Date();
...
function initEvent(){
	...
	$("#searchSjbgBtn").on("click",function(){
		...
		ifHaveData();
		    ...
		    $("#pdfContainer").attr("src","/cmwa/resource/sjjk/previewQdcjWord.html");
		}
	});
		...
		//点击页面汇总导出事件
		 $("#exportHzTable").on("click",function(){
				insertCodeFun ("MAS_hp_cmwa_sjjk_table_export_03");
			  	...
			  	var postData = qdcj.getQueryParam();
			  	var url = ""
			  	if($("#currKhqfTab").val() == "tabkhqf01"){
			  		url = "/qdcj01/exportHzTableData";
				}
				if($("#currKhqfTab").val() == "tabkhqf02"){
					url = "/qdcj02/exportHzTableData";
				}
				if($("#currKhqfTab").val() == "tabkhqf03"){
					url = "/qdcj03/exportHzTableData";
				}
				if($("#currKhqfTab").val() == "tabkhqf04"){
					url = "/qdcj04/exportHzTableData";
				}
				if($("#currKhqfTab").val() == "tabkhqf05"){
					url = "/qdcj05/exportHzTableData";
				}
...
function loadKhqfDialog(){
	...
	$.ajax({
		url: $.fn.cmwaurl()+"/qdcj01/loadQdcjDialog",
		async: true,
		cache: false,
		data : postData,
		dataType: 'json',
...
function loadnthTabs(currId){
	  ...
	  if("qianfeiBtn"==currId){
		  nthTabs.addTab({
				...
				id : 'tabkhqf02',
				title : '集团业务服务费占集团业务收入比重',
				active : true,
				allowClose : false,
			}).addTab({
				id : 'tabkhqf03',
				title : '家庭业务服务费占家庭业务收入比重',
				allowClose : false,
			}).addTab({
				id : 'tabkhqf04',
				title : '激励酬金占总酬金比重',
...
function ifHaveData(){
	if($("#currKhqfTab").val() == "tabkhqf01"){
		qdcj.ifHaveDataAll("/qdcj01");
	}
	if($("#currKhqfTab").val() == "tabkhqf02"){
		qdcj.ifHaveDataAll("/qdcj02");
	}
	if($("#currKhqfTab").val() == "tabkhqf03"){
		qdcj.ifHaveDataAll("/qdcj03");
	}
	if($("#currKhqfTab").val() == "tabkhqf04"){
		qdcj.ifHaveDataAll("/qdcj04");
	}
	if($("#currKhqfTab").val() == "tabkhqf05"){
		qdcj.ifHaveDataAll("/qdcj05");
	}
