...
(function ($) {
	$.fn.nthTabs =  function(options){
		...
		var methods = {
			...
			//事件监听
			listen:function(){
				event.onChangeTab().onTabClose().onTabRollLeft().onTabRollRight().onTabList()
			...
			addTab:function (options) {
				...
				var modelDialog = options.modelDialog==undefined ? settings.modelDialog : options.modelDialog;
				active = active ? 'active':'';
				tab.push('<li role="presentation" class="'+active+'">');
				tab.push('<a id="'+options.id+'" class="a-modelDialog" href="#'+options.id+'" data-toggle="tab">');
				tab.push('<span>'+options.title+'</span>');
/*				allowClose ? tab.push('<i class="fa fa-close tab-close"></i>'): tab.push('');
*/				modelDialog ? tab.push('<i class="fa fa-modelDialog tab-modelDialog" code="'+options.id+'" id="'+options.id+'CurrModal">?</i>'): tab.push('');
				tab.push('</a>');
				tab.push('</li>');
			...
			/*	$('#cityIdLeft').val("");
				...
				$("#"+tabId).append('<i id="'+tabId+'CurrModal" code="'+tabId+'" class="fa fa-modelDialog tab-modelDialog">?</i>');
				    for(var i=0;i<$("#"+tabId).parent().siblings().length;i++){
				    	//console.log($("#"+tabId).parent().siblings().eq(i).children().attr("id"));
				    	var otherId=$("#"+tabId).parent().siblings().eq(i).children().attr("id");
				    	if($("#"+otherId).children("i")){
				    		$("#"+otherId).children("i").remove();
				    	}
				    }
				   /* $("#datetimepickerStart").val($.fn.timeStyle($("#currBeginDate").val()));
					$("#datetimepickerEnd").val($.fn.timeStyle($("#currEndDate").val()));
					console.log($("#provinceCode").attr("fieldName"));
		...
		var event = {
				...
				onChangeTab:function(){
					nthTabs.on("click",'#nav_tabs_ul li a',function () {
						methods.changeTab($(this).attr("id"));
